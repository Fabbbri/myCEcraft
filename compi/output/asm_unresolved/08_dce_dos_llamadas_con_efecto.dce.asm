...
main:
    ...
    jal x1, acumular                                    ; pc=0x0038
    add x6, x11, x0                                     ; pc=0x003C
    add x3, x6, x0 ; promote t3                         ; pc=0x0040
    addi x7, x0, 4                                      ; pc=0x0044
    add x11, x7, x0                                     ; pc=0x0048
    sw x3, -16(x17) ; spill t3                          ; pc=0x004C
    jal x1, acumular                                    ; pc=0x0050
    add x8, x11, x0                                     ; pc=0x0054
    add x4, x8, x0 ; promote t5                         ; pc=0x0058
    addiHIGH x10, x0, 0                                 ; pc=0x005C
    addi x10, x10, 32768                                ; pc=0x0060
    lw x9, 0(x10) ; acumulado                           ; pc=0x0064
    add x11, x9, x0                                     ; pc=0x0068
    jal x0, .L_ir_1_main_end                            ; pc=0x006C
.L_ir_1_main_end:
    ; epilogue
    lw x17, 4(x2)                                       ; pc=0x0070
    addi x2, x2, 28                                     ; pc=0x0074
    freeze                                              ; pc=0x0078

acumular:
    ; prologue
    addiSigned x2, x2, -16                              ; pc=0x007C
    sw x1, 0(x2)                                        ; pc=0x0080
    sw x17, 4(x2)                                       ; pc=0x0084
    addi x17, x2, 16                                    ; pc=0x0088

    add x3, x11, x0 ; parametro promovido valor         ; pc=0x008C
    ...
    jal x0, .L_ir_2_acumular_end                        ; pc=0x00BC
.L_ir_2_acumular_end:
    ; epilogue
    lw x1, 0(x2)                                        ; pc=0x00C0
    lw x17, 4(x2)                                       ; pc=0x00C4
    addi x2, x2, 16                                     ; pc=0x00C8
    jalr x1, 0                                          ; pc=0x00CC