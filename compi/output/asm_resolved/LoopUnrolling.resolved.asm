...
;   .L_ir_3_ir_cmp_true = 0x018C
;   .L_ir_4_ir_cmp_end = 0x0190
;   L_for_start_2 = 0x01A4
;   .L_ir_5_ir_cmp_true = 0x01B4
;   .L_ir_6_ir_cmp_end = 0x01B8
;   L_for_end_3 = 0x0228
;   L_for_end_1 = 0x0238
;   .L_ir_2_procesar_bloques_end = 0x0244

; Referencias resueltas
;   pc=0x0000 portalv -> .L_ir_0_enderExit (addr=0x0018, offset=24)
;   pc=0x0138 jal -> procesar_bloques (addr=0x0158, offset=32)
;   pc=0x0148 jal -> .L_ir_1_main_end (addr=0x014C, offset=4)
;   pc=0x0184 blt -> .L_ir_3_ir_cmp_true (addr=0x018C, offset=8)
;   pc=0x0188 jal -> .L_ir_4_ir_cmp_end (addr=0x0190, offset=8)
;   pc=0x0198 beq -> L_for_end_1 (addr=0x0238, offset=160)
;   pc=0x01AC blt -> .L_ir_5_ir_cmp_true (addr=0x01B4, offset=8)
;   pc=0x01B0 jal -> .L_ir_6_ir_cmp_end (addr=0x01B8, offset=8)
;   pc=0x01C0 beq -> L_for_end_3 (addr=0x0228, offset=104)
;   pc=0x0224 jal -> L_for_start_2 (addr=0x01A4, offset=-128)
;   pc=0x0234 jal -> L_for_start_0 (addr=0x017C, offset=-184)
;   pc=0x0240 jal -> .L_ir_2_procesar_bloques_end (addr=0x0244, offset=4)
...
L_for_start_0:
    ...
    blt x3, x7, 8                                       ; pc=0x0184 ; target=.L_ir_3_ir_cmp_true ; addr=0x018C
    jal x0, 8                                           ; pc=0x0188 ; target=.L_ir_4_ir_cmp_end ; addr=0x0190
.L_ir_3_ir_cmp_true:
    addi x8, x0, 1                                      ; pc=0x018C
.L_ir_4_ir_cmp_end:
    sw x8, -20(x17) ; t0                                ; pc=0x0190
    lw x9, -20(x17) ; t0                                ; pc=0x0194
    beq x9, x0, 160                                     ; pc=0x0198 ; target=L_for_end_1 ; addr=0x0238
    addi x10, x0, 0                                     ; pc=0x019C
    add x4, x10, x0 ; promote i                         ; pc=0x01A0
L_for_start_2:
    addi x5, x0, 8                                      ; pc=0x01A4
    addi x6, x0, 0                                      ; pc=0x01A8
    blt x4, x5, 8                                       ; pc=0x01AC ; target=.L_ir_5_ir_cmp_true ; addr=0x01B4
    jal x0, 8                                           ; pc=0x01B0 ; target=.L_ir_6_ir_cmp_end ; addr=0x01B8
.L_ir_5_ir_cmp_true:
    addi x6, x0, 1                                      ; pc=0x01B4
.L_ir_6_ir_cmp_end:
    sw x6, -24(x17) ; t1                                ; pc=0x01B8
    lw x8, -24(x17) ; t1                                ; pc=0x01BC
    beq x8, x0, 104                                     ; pc=0x01C0 ; target=L_for_end_3 ; addr=0x0228
    ...
    jal x0, -128                                        ; pc=0x0224 ; target=L_for_start_2 ; addr=0x01A4
L_for_end_3:
    addi x6, x0, 1                                      ; pc=0x0228
    add x7, x3, x6                                      ; pc=0x022C
    add x3, x7, x0 ; promote b                          ; pc=0x0230
    jal x0, -184                                        ; pc=0x0234 ; target=L_for_start_0 ; addr=0x017C
L_for_end_1:
    lw x9, -8(x17) ; total                              ; pc=0x0238
    add x11, x9, x0                                     ; pc=0x023C
    jal x0, 4                                           ; pc=0x0240 ; target=.L_ir_2_procesar_bloques_end ; addr=0x0244
