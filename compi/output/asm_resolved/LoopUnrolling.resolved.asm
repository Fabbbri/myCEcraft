...
;   .L_ir_3_ir_cmp_true = 0x028C
;   .L_ir_4_ir_cmp_end = 0x0290
;   L_for_start_2 = 0x02A4
;   .L_ir_5_ir_cmp_true = 0x02B4
;   .L_ir_6_ir_cmp_end = 0x02B8
;   L_for_end_3 = 0x0328
;   L_for_end_1 = 0x0338
;   .L_ir_2_procesar_bloques_end = 0x0344

; Referencias resueltas
;   pc=0x0000 portalv -> .L_ir_0_enderExit (addr=0x0018, offset=24)
;   pc=0x0238 jal -> procesar_bloques (addr=0x0258, offset=32)
;   pc=0x0248 jal -> .L_ir_1_main_end (addr=0x024C, offset=4)
;   pc=0x0284 blt -> .L_ir_3_ir_cmp_true (addr=0x028C, offset=8)
;   pc=0x0288 jal -> .L_ir_4_ir_cmp_end (addr=0x0290, offset=8)
;   pc=0x0298 beq -> L_for_end_1 (addr=0x0338, offset=160)
;   pc=0x02AC blt -> .L_ir_5_ir_cmp_true (addr=0x02B4, offset=8)
;   pc=0x02B0 jal -> .L_ir_6_ir_cmp_end (addr=0x02B8, offset=8)
;   pc=0x02C0 beq -> L_for_end_3 (addr=0x0328, offset=104)
;   pc=0x0324 jal -> L_for_start_2 (addr=0x02A4, offset=-128)
;   pc=0x0334 jal -> L_for_start_0 (addr=0x027C, offset=-184)
;   pc=0x0340 jal -> .L_ir_2_procesar_bloques_end (addr=0x0344, offset=4)
...
L_for_start_0:
    ...
    blt x3, x7, 8                                       ; pc=0x0284 ; target=.L_ir_3_ir_cmp_true ; addr=0x028C
    jal x0, 8                                           ; pc=0x0288 ; target=.L_ir_4_ir_cmp_end ; addr=0x0290
.L_ir_3_ir_cmp_true:
    addi x8, x0, 1                                      ; pc=0x028C
.L_ir_4_ir_cmp_end:
    sw x8, -20(x17) ; t0                                ; pc=0x0290
    lw x9, -20(x17) ; t0                                ; pc=0x0294
    beq x9, x0, 160                                     ; pc=0x0298 ; target=L_for_end_1 ; addr=0x0338
    addi x10, x0, 0                                     ; pc=0x029C
    add x4, x10, x0 ; promote i                         ; pc=0x02A0
L_for_start_2:
    addi x5, x0, 8                                      ; pc=0x02A4
    addi x6, x0, 0                                      ; pc=0x02A8
    blt x4, x5, 8                                       ; pc=0x02AC ; target=.L_ir_5_ir_cmp_true ; addr=0x02B4
    jal x0, 8                                           ; pc=0x02B0 ; target=.L_ir_6_ir_cmp_end ; addr=0x02B8
.L_ir_5_ir_cmp_true:
    addi x6, x0, 1                                      ; pc=0x02B4
.L_ir_6_ir_cmp_end:
    sw x6, -24(x17) ; t1                                ; pc=0x02B8
    lw x8, -24(x17) ; t1                                ; pc=0x02BC
    beq x8, x0, 104                                     ; pc=0x02C0 ; target=L_for_end_3 ; addr=0x0328
    ...
    jal x0, -128                                        ; pc=0x0324 ; target=L_for_start_2 ; addr=0x02A4
L_for_end_3:
    addi x6, x0, 1                                      ; pc=0x0328
    add x7, x3, x6                                      ; pc=0x032C
    add x3, x7, x0 ; promote b                          ; pc=0x0330
    jal x0, -184                                        ; pc=0x0334 ; target=L_for_start_0 ; addr=0x027C
L_for_end_1:
    lw x9, -8(x17) ; total                              ; pc=0x0338
    add x11, x9, x0                                     ; pc=0x033C
    jal x0, 4                                           ; pc=0x0340 ; target=.L_ir_2_procesar_bloques_end ; addr=0x0344
