...
;   L_for_start_2 = 0x00C0
;   .L_ir_4_ir_cmp_true = 0x00D0
;   .L_ir_5_ir_cmp_end = 0x00D4
;   L_for_start_4 = 0x00F0
;   .L_ir_6_ir_cmp_true = 0x0104
;   .L_ir_7_ir_cmp_end = 0x0108
;   L_for_end_5 = 0x01B8
;   L_for_end_3 = 0x01FC
;   L_for_end_1 = 0x020C
;   .L_ir_1_main_end = 0x02B8

; Referencias resueltas
;   pc=0x0000 portalv -> .L_ir_0_enderExit (addr=0x0018, offset=24)
;   pc=0x00A0 blt -> .L_ir_2_ir_cmp_true (addr=0x00A8, offset=8)
;   pc=0x00A4 jal -> .L_ir_3_ir_cmp_end (addr=0x00AC, offset=8)
;   pc=0x00B4 beq -> L_for_end_1 (addr=0x020C, offset=344)
;   pc=0x00C8 blt -> .L_ir_4_ir_cmp_true (addr=0x00D0, offset=8)
;   pc=0x00CC jal -> .L_ir_5_ir_cmp_end (addr=0x00D4, offset=8)
;   pc=0x00DC beq -> L_for_end_3 (addr=0x01FC, offset=288)
;   pc=0x00FC blt -> .L_ir_6_ir_cmp_true (addr=0x0104, offset=8)
;   pc=0x0100 jal -> .L_ir_7_ir_cmp_end (addr=0x0108, offset=8)
;   pc=0x0110 beq -> L_for_end_5 (addr=0x01B8, offset=168)
;   pc=0x01B4 jal -> L_for_start_4 (addr=0x00F0, offset=-196)
;   pc=0x01F8 jal -> L_for_start_2 (addr=0x00C0, offset=-312)
;   pc=0x0208 jal -> L_for_start_0 (addr=0x0098, offset=-368)
;   pc=0x02B4 jal -> .L_ir_1_main_end (addr=0x02B8, offset=4)
...
.L_ir_3_ir_cmp_end:
    ...
    beq x8, x0, 344                                     ; pc=0x00B4 ; target=L_for_end_1 ; addr=0x020C
    addi x9, x0, 0                                      ; pc=0x00B8
    add x4, x9, x0 ; promote j                          ; pc=0x00BC
L_for_start_2:
    addi x10, x0, 2                                     ; pc=0x00C0
    addi x5, x0, 0                                      ; pc=0x00C4
    blt x4, x10, 8                                      ; pc=0x00C8 ; target=.L_ir_4_ir_cmp_true ; addr=0x00D0
    jal x0, 8                                           ; pc=0x00CC ; target=.L_ir_5_ir_cmp_end ; addr=0x00D4
.L_ir_4_ir_cmp_true:
    addi x5, x0, 1                                      ; pc=0x00D0
.L_ir_5_ir_cmp_end:
    sw x5, -100(x17) ; t2                               ; pc=0x00D4
    lw x7, -100(x17) ; t2                               ; pc=0x00D8
    beq x7, x0, 288                                     ; pc=0x00DC ; target=L_for_end_3 ; addr=0x01FC
...
L_for_start_4:
    ...
    blt x9, x5, 8                                       ; pc=0x00FC ; target=.L_ir_6_ir_cmp_true ; addr=0x0104
    jal x0, 8                                           ; pc=0x0100 ; target=.L_ir_7_ir_cmp_end ; addr=0x0108
.L_ir_6_ir_cmp_true:
    addi x10, x0, 1                                     ; pc=0x0104
.L_ir_7_ir_cmp_end:
    sw x10, -104(x17) ; t4                              ; pc=0x0108
    lw x7, -104(x17) ; t4                               ; pc=0x010C
    beq x7, x0, 168                                     ; pc=0x0110 ; target=L_for_end_5 ; addr=0x01B8
    ...
    jal x0, -196                                        ; pc=0x01B4 ; target=L_for_start_4 ; addr=0x00F0
L_for_end_5:
    ...
    jal x0, -312                                        ; pc=0x01F8 ; target=L_for_start_2 ; addr=0x00C0
L_for_end_3:
    addi x10, x0, 1                                     ; pc=0x01FC
    add x8, x3, x10                                     ; pc=0x0200
    add x3, x8, x0 ; promote i                          ; pc=0x0204
    jal x0, -368                                        ; pc=0x0208 ; target=L_for_start_0 ; addr=0x0098
L_for_end_1:
    ...
    jal x0, 4                                           ; pc=0x02B4 ; target=.L_ir_1_main_end ; addr=0x02B8
.L_ir_1_main_end:
    ; epilogue
