...
;   main = 0x001C
;   .L_codegen_1_main_end = 0x0094
;   fact = 0x00CC
;   .L0_if_else = 0x0150
;   .L1_if_end = 0x0150
;   .L_codegen_2_fact_end = 0x01E0

; Referencias resueltas
;   pc=0x0000 portalv -> .L_codegen_0_enderExit (addr=0x001C, offset=28)
;   pc=0x0064 jal -> fact (addr=0x00CC, offset=104)
;   pc=0x008C jal -> .L_codegen_1_main_end (addr=0x0094, offset=8)
;   pc=0x0118 blt -> .L0_if_else (addr=0x0150, offset=56)
;   pc=0x0140 jal -> .L_codegen_2_fact_end (addr=0x01E0, offset=160)
;   pc=0x0148 jal -> .L1_if_end (addr=0x0150, offset=8)
;   pc=0x01A0 jal -> fact (addr=0x00CC, offset=-212)
;   pc=0x01D8 jal -> .L_codegen_2_fact_end (addr=0x01E0, offset=8)
...
main:
    ...
    jal x1, 104                                         ; pc=0x0064 ; target=fact ; addr=0x00CC
    ...
    jal x0, 8                                           ; pc=0x008C ; target=.L_codegen_1_main_end ; addr=0x0094
...
fact:
    ...
    sw x11, 8(x17) ; parámetro n                        ; pc=0x00F4


    ; if
    lw x3, 8(x17) ; n                                   ; pc=0x00F8
...
.L_codegen_2_fact_end:
    ...
    addi x2, x2, 8                                      ; pc=0x0200
    sleep ; stall RAW                                   ; pc=0x0204
    sleep ; stall RAW                                   ; pc=0x0208
    sleep ; stall RAW                                   ; pc=0x020C
    jalr x1, 0                                          ; pc=0x0210
    sleep ; nop despues de control                      ; pc=0x0214

    ; final de programa
    addiHIGH x1, x0, 0xDEAD                             ; pc=0x0218
