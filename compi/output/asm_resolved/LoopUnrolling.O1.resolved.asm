...
;   pc=0x00D0 blt -> .L_ir_3_ir_cmp_true (addr=0x00D8, offset=8)
;   pc=0x00D4 jal -> .L_ir_4_ir_cmp_end (addr=0x00DC, offset=8)
;   pc=0x00E4 beq -> L_for_end_1 (addr=0x0248, offset=356)
;   pc=0x00F8 blt -> .L_ir_5_ir_cmp_true (addr=0x0100, offset=8)
;   pc=0x00FC jal -> .L_ir_6_ir_cmp_end (addr=0x0104, offset=8)
;   pc=0x010C beq -> L_for_end_3 (addr=0x01D0, offset=196)
;   pc=0x01CC jal -> L_for_start_2 (addr=0x00F0, offset=-220)
;   pc=0x0244 jal -> L_for_start_0 (addr=0x00C4, offset=-384)
...
L_for_start_0:
    ...
    blt x7, x8, 8                                       ; pc=0x00D0 ; target=.L_ir_3_ir_cmp_true ; addr=0x00D8
...
.L_ir_4_ir_cmp_end:
    ...
    beq x10, x0, 356                                    ; pc=0x00E4 ; target=L_for_end_1 ; addr=0x0248
    addi x5, x0, 0                                      ; pc=0x00E8
    add x4, x5, x0 ; promote i                          ; pc=0x00EC
L_for_start_2:
    addi x6, x0, 2                                      ; pc=0x00F0
    addi x9, x0, 0                                      ; pc=0x00F4
    blt x4, x6, 8                                       ; pc=0x00F8 ; target=.L_ir_5_ir_cmp_true ; addr=0x0100
...
.L_ir_6_ir_cmp_end:
    ...
    beq x8, x0, 196                                     ; pc=0x010C ; target=L_for_end_3 ; addr=0x01D0
    ...
    jal x0, -220                                        ; pc=0x01CC ; target=L_for_start_2 ; addr=0x00F0
L_for_end_3:
    ...
    jal x0, -384                                        ; pc=0x0244 ; target=L_for_start_0 ; addr=0x00C4
L_for_end_1:
