...
;   tea_encrypt = 0x0244
;   L_for_start_0 = 0x02DC
;   .L_ir_7_ir_cmp_true = 0x02EC
;   .L_ir_8_ir_cmp_end = 0x02F0
;   L_for_end_1 = 0x042C
;   .L_ir_6_tea_encrypt_end = 0x045C
;   tea_decrypt = 0x049C
;   L_for_start_2 = 0x053C
;   .L_ir_10_ir_cmp_true = 0x054C
;   .L_ir_11_ir_cmp_end = 0x0550
;   L_for_end_3 = 0x0688
;   .L_ir_9_tea_decrypt_end = 0x06B8

; Referencias resueltas
;   pc=0x0000 portalv -> .L_ir_0_enderExit (addr=0x0018, offset=24)
;   pc=0x00C8 jal -> tea_encrypt (addr=0x0244, offset=380)
;   pc=0x0164 jal -> tea_decrypt (addr=0x049C, offset=824)
...
;   pc=0x02E4 blt -> .L_ir_7_ir_cmp_true (addr=0x02EC, offset=8)
;   pc=0x02E8 jal -> .L_ir_8_ir_cmp_end (addr=0x02F0, offset=8)
;   pc=0x02F4 beq -> L_for_end_1 (addr=0x042C, offset=312)
;   pc=0x0428 jal -> L_for_start_0 (addr=0x02DC, offset=-332)
;   pc=0x0544 blt -> .L_ir_10_ir_cmp_true (addr=0x054C, offset=8)
;   pc=0x0548 jal -> .L_ir_11_ir_cmp_end (addr=0x0550, offset=8)
;   pc=0x0554 beq -> L_for_end_3 (addr=0x0688, offset=308)
;   pc=0x0684 jal -> L_for_start_2 (addr=0x053C, offset=-328)
...
main:
    ...
    jal x1, 824                                         ; pc=0x0164 ; target=tea_decrypt ; addr=0x049C
...
L_for_start_0:
    ...
    blt x21, x10, 8                                     ; pc=0x02E4 ; target=.L_ir_7_ir_cmp_true ; addr=0x02EC
    jal x0, 8                                           ; pc=0x02E8 ; target=.L_ir_8_ir_cmp_end ; addr=0x02F0
.L_ir_7_ir_cmp_true:
    addi x9, x0, 1                                      ; pc=0x02EC
.L_ir_8_ir_cmp_end:
    add x30, x9, x0 ; promote t2                        ; pc=0x02F0
    beq x30, x0, 312                                    ; pc=0x02F4 ; target=L_for_end_1 ; addr=0x042C
    ...
    jal x0, -332                                        ; pc=0x0428 ; target=L_for_start_0 ; addr=0x02DC
L_for_end_1:
...
L_for_start_2:
    ...
    blt x21, x9, 8                                      ; pc=0x0544 ; target=.L_ir_10_ir_cmp_true ; addr=0x054C
    jal x0, 8                                           ; pc=0x0548 ; target=.L_ir_11_ir_cmp_end ; addr=0x0550
.L_ir_10_ir_cmp_true:
    addi x6, x0, 1                                      ; pc=0x054C
.L_ir_11_ir_cmp_end:
    add x30, x6, x0 ; promote t23                       ; pc=0x0550
    beq x30, x0, 308                                    ; pc=0x0554 ; target=L_for_end_3 ; addr=0x0688
    ...
    jal x0, -328                                        ; pc=0x0684 ; target=L_for_start_2 ; addr=0x053C
L_for_end_3:
