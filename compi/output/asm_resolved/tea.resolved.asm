; ==================================================
; Fase 5 - saltos y referencias resueltas
; Convencion: offset relativo en bytes = target_pc - current_pc
; ==================================================

; Tabla de etiquetas
;   .L_codegen_0_enderExit = 0x0020
;   main = 0x0020
;   .L6_if_else = 0x06A0
;   .L7_if_end = 0x06A0
;   .L4_if_else = 0x06AC
;   .L5_if_end = 0x06AC
;   .L_codegen_1_main_end = 0x06D8
;   tea_encrypt = 0x06FC
;   .L0_for_start = 0x081C
;   .L1_for_end = 0x0CBC
;   .L_codegen_2_tea_encrypt_end = 0x0D84
;   tea_decrypt = 0x0DC0
;   .L2_for_start = 0x0F00
;   .L3_for_end = 0x13A0
;   .L_codegen_3_tea_decrypt_end = 0x1468

; Referencias resueltas
;   pc=0x0000 portalv -> .L_codegen_0_enderExit (addr=0x0020, offset=32)
;   pc=0x0270 jal -> tea_encrypt (addr=0x06FC, offset=1164)
;   pc=0x0484 jal -> tea_decrypt (addr=0x0DC0, offset=2364)
;   pc=0x0570 bne -> .L4_if_else (addr=0x06AC, offset=316)
;   pc=0x065C bne -> .L6_if_else (addr=0x06A0, offset=68)
;   pc=0x0688 jal -> .L_codegen_1_main_end (addr=0x06D8, offset=80)
;   pc=0x0694 jal -> .L7_if_end (addr=0x06A0, offset=12)
;   pc=0x06A0 jal -> .L5_if_end (addr=0x06AC, offset=12)
;   pc=0x06CC jal -> .L_codegen_1_main_end (addr=0x06D8, offset=12)
;   pc=0x083C bge -> .L1_for_end (addr=0x0CBC, offset=1152)
;   pc=0x0CB0 jal -> .L0_for_start (addr=0x081C, offset=-1172)
;   pc=0x0F20 bge -> .L3_for_end (addr=0x13A0, offset=1152)
;   pc=0x1394 jal -> .L2_for_start (addr=0x0F00, offset=-1172)

; ==================================================
; Ensamblador generado para Craft21
; Fase 4 - versión inicial
; ==================================================

.data
key: ; addr=0x8000
    .word 0
    .word 1
    .word 2
    .word 3
plain: ; addr=0x8010
    .word 0
    .word 0
cipher: ; addr=0x8018
    .word 0
    .word 0
roundtrip: ; addr=0x8020
    .word 0
    .word 0
DELTA: ; addr=0x8028
    .word 0x9E3779B9
SUM_INIT: ; addr=0x802C
    .word 0xC6EF3720

.text

    ; @EnterCraftWorld
    portalv x0, x0, 32                                  ; pc=0x0000 ; target=.L_codegen_0_enderExit ; addr=0x0020
    sleep ; nop despues de control                      ; pc=0x0004
    sleep ; nop despues de control                      ; pc=0x0008
    lwv v0, 0(v0)                                       ; pc=0x000C
    sleep ; stall RAW                                   ; pc=0x0010
    sleep ; stall RAW                                   ; pc=0x0014
    sleep ; stall RAW                                   ; pc=0x0018
    closev ; cerrar Secure Mode despues del bootstrap   ; pc=0x001C
.L_codegen_0_enderExit:

main:
    ; inicializar stack pointer
    addiHIGH x2, x0, 0                                  ; pc=0x0020
    sleep ; stall RAW                                   ; pc=0x0024
    sleep ; stall RAW                                   ; pc=0x0028
    sleep ; stall RAW                                   ; pc=0x002C
    addi x2, x2, 0x7FF0                                 ; pc=0x0030
    sleep ; stall RAW                                   ; pc=0x0034
    sleep ; stall RAW                                   ; pc=0x0038
    sleep ; stall RAW                                   ; pc=0x003C

    ; prologue
    addiSigned x2, x2, -8                               ; pc=0x0040
    sleep ; stall RAW                                   ; pc=0x0044
    sleep ; stall RAW                                   ; pc=0x0048
    sleep ; stall RAW                                   ; pc=0x004C
    sw x1, 0(x2)                                        ; pc=0x0050
    sw x17, 4(x2)                                       ; pc=0x0054
    addi x17, x2, 8                                     ; pc=0x0058
    sleep ; stall RAW                                   ; pc=0x005C
    sleep ; stall RAW                                   ; pc=0x0060
    sleep ; stall RAW                                   ; pc=0x0064

    addi x3, x0, 0                                      ; pc=0x0068
    sleep ; stall RAW                                   ; pc=0x006C
    sleep ; stall RAW                                   ; pc=0x0070
    sleep ; stall RAW                                   ; pc=0x0074
    add x4, x3, x3                                      ; pc=0x0078
    sleep ; stall RAW                                   ; pc=0x007C
    sleep ; stall RAW                                   ; pc=0x0080
    sleep ; stall RAW                                   ; pc=0x0084
    add x4, x4, x4                                      ; pc=0x0088
    sleep ; stall RAW                                   ; pc=0x008C
    sleep ; stall RAW                                   ; pc=0x0090
    sleep ; stall RAW                                   ; pc=0x0094
    addiHIGH x5, x0, 0                                  ; pc=0x0098
    sleep ; stall RAW                                   ; pc=0x009C
    sleep ; stall RAW                                   ; pc=0x00A0
    sleep ; stall RAW                                   ; pc=0x00A4
    addi x5, x5, 32784                                  ; pc=0x00A8
    sleep ; stall RAW                                   ; pc=0x00AC
    sleep ; stall RAW                                   ; pc=0x00B0
    sleep ; stall RAW                                   ; pc=0x00B4
    ; base plain
    add x5, x5, x4                                      ; pc=0x00B8
    sleep ; stall RAW                                   ; pc=0x00BC
    sleep ; stall RAW                                   ; pc=0x00C0
    sleep ; stall RAW                                   ; pc=0x00C4
    lw x4, 0(x5)                                        ; pc=0x00C8
    sleep ; stall RAW                                   ; pc=0x00CC
    sleep ; stall RAW                                   ; pc=0x00D0
    sleep ; stall RAW                                   ; pc=0x00D4
    addi x5, x0, 0                                      ; pc=0x00D8
    sleep ; stall RAW                                   ; pc=0x00DC
    sleep ; stall RAW                                   ; pc=0x00E0
    sleep ; stall RAW                                   ; pc=0x00E4
    add x3, x5, x5                                      ; pc=0x00E8
    sleep ; stall RAW                                   ; pc=0x00EC
    sleep ; stall RAW                                   ; pc=0x00F0
    sleep ; stall RAW                                   ; pc=0x00F4
    add x3, x3, x3                                      ; pc=0x00F8
    sleep ; stall RAW                                   ; pc=0x00FC
    sleep ; stall RAW                                   ; pc=0x0100
    sleep ; stall RAW                                   ; pc=0x0104
    addiHIGH x6, x0, 0                                  ; pc=0x0108
    sleep ; stall RAW                                   ; pc=0x010C
    sleep ; stall RAW                                   ; pc=0x0110
    sleep ; stall RAW                                   ; pc=0x0114
    addi x6, x6, 32792                                  ; pc=0x0118
    sleep ; stall RAW                                   ; pc=0x011C
    sleep ; stall RAW                                   ; pc=0x0120
    sleep ; stall RAW                                   ; pc=0x0124
    ; base cipher
    add x6, x6, x3                                      ; pc=0x0128
    sleep ; stall RAW                                   ; pc=0x012C
    sleep ; stall RAW                                   ; pc=0x0130
    sleep ; stall RAW                                   ; pc=0x0134
    sw x4, 0(x6)                                        ; pc=0x0138
    addi x4, x0, 1                                      ; pc=0x013C
    sleep ; stall RAW                                   ; pc=0x0140
    sleep ; stall RAW                                   ; pc=0x0144
    sleep ; stall RAW                                   ; pc=0x0148
    add x6, x4, x4                                      ; pc=0x014C
    sleep ; stall RAW                                   ; pc=0x0150
    sleep ; stall RAW                                   ; pc=0x0154
    sleep ; stall RAW                                   ; pc=0x0158
    add x6, x6, x6                                      ; pc=0x015C
    sleep ; stall RAW                                   ; pc=0x0160
    sleep ; stall RAW                                   ; pc=0x0164
    sleep ; stall RAW                                   ; pc=0x0168
    addiHIGH x3, x0, 0                                  ; pc=0x016C
    sleep ; stall RAW                                   ; pc=0x0170
    sleep ; stall RAW                                   ; pc=0x0174
    sleep ; stall RAW                                   ; pc=0x0178
    addi x3, x3, 32784                                  ; pc=0x017C
    sleep ; stall RAW                                   ; pc=0x0180
    sleep ; stall RAW                                   ; pc=0x0184
    sleep ; stall RAW                                   ; pc=0x0188
    ; base plain
    add x3, x3, x6                                      ; pc=0x018C
    sleep ; stall RAW                                   ; pc=0x0190
    sleep ; stall RAW                                   ; pc=0x0194
    sleep ; stall RAW                                   ; pc=0x0198
    lw x6, 0(x3)                                        ; pc=0x019C
    sleep ; stall RAW                                   ; pc=0x01A0
    sleep ; stall RAW                                   ; pc=0x01A4
    sleep ; stall RAW                                   ; pc=0x01A8
    addi x3, x0, 1                                      ; pc=0x01AC
    sleep ; stall RAW                                   ; pc=0x01B0
    sleep ; stall RAW                                   ; pc=0x01B4
    sleep ; stall RAW                                   ; pc=0x01B8
    add x4, x3, x3                                      ; pc=0x01BC
    sleep ; stall RAW                                   ; pc=0x01C0
    sleep ; stall RAW                                   ; pc=0x01C4
    sleep ; stall RAW                                   ; pc=0x01C8
    add x4, x4, x4                                      ; pc=0x01CC
    sleep ; stall RAW                                   ; pc=0x01D0
    sleep ; stall RAW                                   ; pc=0x01D4
    sleep ; stall RAW                                   ; pc=0x01D8
    addiHIGH x5, x0, 0                                  ; pc=0x01DC
    sleep ; stall RAW                                   ; pc=0x01E0
    sleep ; stall RAW                                   ; pc=0x01E4
    sleep ; stall RAW                                   ; pc=0x01E8
    addi x5, x5, 32792                                  ; pc=0x01EC
    sleep ; stall RAW                                   ; pc=0x01F0
    sleep ; stall RAW                                   ; pc=0x01F4
    sleep ; stall RAW                                   ; pc=0x01F8
    ; base cipher
    add x5, x5, x4                                      ; pc=0x01FC
    sleep ; stall RAW                                   ; pc=0x0200
    sleep ; stall RAW                                   ; pc=0x0204
    sleep ; stall RAW                                   ; pc=0x0208
    sw x6, 0(x5)                                        ; pc=0x020C
    addiHIGH x6, x0, 0                                  ; pc=0x0210
    sleep ; stall RAW                                   ; pc=0x0214
    sleep ; stall RAW                                   ; pc=0x0218
    sleep ; stall RAW                                   ; pc=0x021C
    addi x6, x6, 32792                                  ; pc=0x0220
    sleep ; stall RAW                                   ; pc=0x0224
    sleep ; stall RAW                                   ; pc=0x0228
    sleep ; stall RAW                                   ; pc=0x022C
    ; base cipher
    add x11, x6, x0                                     ; pc=0x0230
    sleep ; stall RAW                                   ; pc=0x0234
    sleep ; stall RAW                                   ; pc=0x0238
    sleep ; stall RAW                                   ; pc=0x023C
    addiHIGH x6, x0, 0                                  ; pc=0x0240
    sleep ; stall RAW                                   ; pc=0x0244
    sleep ; stall RAW                                   ; pc=0x0248
    sleep ; stall RAW                                   ; pc=0x024C
    addi x6, x6, 32768                                  ; pc=0x0250
    sleep ; stall RAW                                   ; pc=0x0254
    sleep ; stall RAW                                   ; pc=0x0258
    sleep ; stall RAW                                   ; pc=0x025C
    ; base key
    add x12, x6, x0                                     ; pc=0x0260
    sleep ; stall RAW                                   ; pc=0x0264
    sleep ; stall RAW                                   ; pc=0x0268
    sleep ; stall RAW                                   ; pc=0x026C
    jal x1, 1164                                        ; pc=0x0270 ; target=tea_encrypt ; addr=0x06FC
    sleep ; nop despues de control                      ; pc=0x0274
    sleep ; nop despues de control                      ; pc=0x0278
    addi x6, x0, 0                                      ; pc=0x027C
    sleep ; stall RAW                                   ; pc=0x0280
    sleep ; stall RAW                                   ; pc=0x0284
    sleep ; stall RAW                                   ; pc=0x0288
    add x5, x6, x6                                      ; pc=0x028C
    sleep ; stall RAW                                   ; pc=0x0290
    sleep ; stall RAW                                   ; pc=0x0294
    sleep ; stall RAW                                   ; pc=0x0298
    add x5, x5, x5                                      ; pc=0x029C
    sleep ; stall RAW                                   ; pc=0x02A0
    sleep ; stall RAW                                   ; pc=0x02A4
    sleep ; stall RAW                                   ; pc=0x02A8
    addiHIGH x4, x0, 0                                  ; pc=0x02AC
    sleep ; stall RAW                                   ; pc=0x02B0
    sleep ; stall RAW                                   ; pc=0x02B4
    sleep ; stall RAW                                   ; pc=0x02B8
    addi x4, x4, 32792                                  ; pc=0x02BC
    sleep ; stall RAW                                   ; pc=0x02C0
    sleep ; stall RAW                                   ; pc=0x02C4
    sleep ; stall RAW                                   ; pc=0x02C8
    ; base cipher
    add x4, x4, x5                                      ; pc=0x02CC
    sleep ; stall RAW                                   ; pc=0x02D0
    sleep ; stall RAW                                   ; pc=0x02D4
    sleep ; stall RAW                                   ; pc=0x02D8
    lw x5, 0(x4)                                        ; pc=0x02DC
    sleep ; stall RAW                                   ; pc=0x02E0
    sleep ; stall RAW                                   ; pc=0x02E4
    sleep ; stall RAW                                   ; pc=0x02E8
    addi x4, x0, 0                                      ; pc=0x02EC
    sleep ; stall RAW                                   ; pc=0x02F0
    sleep ; stall RAW                                   ; pc=0x02F4
    sleep ; stall RAW                                   ; pc=0x02F8
    add x6, x4, x4                                      ; pc=0x02FC
    sleep ; stall RAW                                   ; pc=0x0300
    sleep ; stall RAW                                   ; pc=0x0304
    sleep ; stall RAW                                   ; pc=0x0308
    add x6, x6, x6                                      ; pc=0x030C
    sleep ; stall RAW                                   ; pc=0x0310
    sleep ; stall RAW                                   ; pc=0x0314
    sleep ; stall RAW                                   ; pc=0x0318
    addiHIGH x3, x0, 0                                  ; pc=0x031C
    sleep ; stall RAW                                   ; pc=0x0320
    sleep ; stall RAW                                   ; pc=0x0324
    sleep ; stall RAW                                   ; pc=0x0328
    addi x3, x3, 32800                                  ; pc=0x032C
    sleep ; stall RAW                                   ; pc=0x0330
    sleep ; stall RAW                                   ; pc=0x0334
    sleep ; stall RAW                                   ; pc=0x0338
    ; base roundtrip
    add x3, x3, x6                                      ; pc=0x033C
    sleep ; stall RAW                                   ; pc=0x0340
    sleep ; stall RAW                                   ; pc=0x0344
    sleep ; stall RAW                                   ; pc=0x0348
    sw x5, 0(x3)                                        ; pc=0x034C
    addi x5, x0, 1                                      ; pc=0x0350
    sleep ; stall RAW                                   ; pc=0x0354
    sleep ; stall RAW                                   ; pc=0x0358
    sleep ; stall RAW                                   ; pc=0x035C
    add x3, x5, x5                                      ; pc=0x0360
    sleep ; stall RAW                                   ; pc=0x0364
    sleep ; stall RAW                                   ; pc=0x0368
    sleep ; stall RAW                                   ; pc=0x036C
    add x3, x3, x3                                      ; pc=0x0370
    sleep ; stall RAW                                   ; pc=0x0374
    sleep ; stall RAW                                   ; pc=0x0378
    sleep ; stall RAW                                   ; pc=0x037C
    addiHIGH x6, x0, 0                                  ; pc=0x0380
    sleep ; stall RAW                                   ; pc=0x0384
    sleep ; stall RAW                                   ; pc=0x0388
    sleep ; stall RAW                                   ; pc=0x038C
    addi x6, x6, 32792                                  ; pc=0x0390
    sleep ; stall RAW                                   ; pc=0x0394
    sleep ; stall RAW                                   ; pc=0x0398
    sleep ; stall RAW                                   ; pc=0x039C
    ; base cipher
    add x6, x6, x3                                      ; pc=0x03A0
    sleep ; stall RAW                                   ; pc=0x03A4
    sleep ; stall RAW                                   ; pc=0x03A8
    sleep ; stall RAW                                   ; pc=0x03AC
    lw x3, 0(x6)                                        ; pc=0x03B0
    sleep ; stall RAW                                   ; pc=0x03B4
    sleep ; stall RAW                                   ; pc=0x03B8
    sleep ; stall RAW                                   ; pc=0x03BC
    addi x6, x0, 1                                      ; pc=0x03C0
    sleep ; stall RAW                                   ; pc=0x03C4
    sleep ; stall RAW                                   ; pc=0x03C8
    sleep ; stall RAW                                   ; pc=0x03CC
    add x5, x6, x6                                      ; pc=0x03D0
    sleep ; stall RAW                                   ; pc=0x03D4
    sleep ; stall RAW                                   ; pc=0x03D8
    sleep ; stall RAW                                   ; pc=0x03DC
    add x5, x5, x5                                      ; pc=0x03E0
    sleep ; stall RAW                                   ; pc=0x03E4
    sleep ; stall RAW                                   ; pc=0x03E8
    sleep ; stall RAW                                   ; pc=0x03EC
    addiHIGH x4, x0, 0                                  ; pc=0x03F0
    sleep ; stall RAW                                   ; pc=0x03F4
    sleep ; stall RAW                                   ; pc=0x03F8
    sleep ; stall RAW                                   ; pc=0x03FC
    addi x4, x4, 32800                                  ; pc=0x0400
    sleep ; stall RAW                                   ; pc=0x0404
    sleep ; stall RAW                                   ; pc=0x0408
    sleep ; stall RAW                                   ; pc=0x040C
    ; base roundtrip
    add x4, x4, x5                                      ; pc=0x0410
    sleep ; stall RAW                                   ; pc=0x0414
    sleep ; stall RAW                                   ; pc=0x0418
    sleep ; stall RAW                                   ; pc=0x041C
    sw x3, 0(x4)                                        ; pc=0x0420
    addiHIGH x3, x0, 0                                  ; pc=0x0424
    sleep ; stall RAW                                   ; pc=0x0428
    sleep ; stall RAW                                   ; pc=0x042C
    sleep ; stall RAW                                   ; pc=0x0430
    addi x3, x3, 32800                                  ; pc=0x0434
    sleep ; stall RAW                                   ; pc=0x0438
    sleep ; stall RAW                                   ; pc=0x043C
    sleep ; stall RAW                                   ; pc=0x0440
    ; base roundtrip
    add x11, x3, x0                                     ; pc=0x0444
    sleep ; stall RAW                                   ; pc=0x0448
    sleep ; stall RAW                                   ; pc=0x044C
    sleep ; stall RAW                                   ; pc=0x0450
    addiHIGH x3, x0, 0                                  ; pc=0x0454
    sleep ; stall RAW                                   ; pc=0x0458
    sleep ; stall RAW                                   ; pc=0x045C
    sleep ; stall RAW                                   ; pc=0x0460
    addi x3, x3, 32768                                  ; pc=0x0464
    sleep ; stall RAW                                   ; pc=0x0468
    sleep ; stall RAW                                   ; pc=0x046C
    sleep ; stall RAW                                   ; pc=0x0470
    ; base key
    add x12, x3, x0                                     ; pc=0x0474
    sleep ; stall RAW                                   ; pc=0x0478
    sleep ; stall RAW                                   ; pc=0x047C
    sleep ; stall RAW                                   ; pc=0x0480
    jal x1, 2364                                        ; pc=0x0484 ; target=tea_decrypt ; addr=0x0DC0
    sleep ; nop despues de control                      ; pc=0x0488
    sleep ; nop despues de control                      ; pc=0x048C

    ; if
    addi x3, x0, 0                                      ; pc=0x0490
    sleep ; stall RAW                                   ; pc=0x0494
    sleep ; stall RAW                                   ; pc=0x0498
    sleep ; stall RAW                                   ; pc=0x049C
    add x4, x3, x3                                      ; pc=0x04A0
    sleep ; stall RAW                                   ; pc=0x04A4
    sleep ; stall RAW                                   ; pc=0x04A8
    sleep ; stall RAW                                   ; pc=0x04AC
    add x4, x4, x4                                      ; pc=0x04B0
    sleep ; stall RAW                                   ; pc=0x04B4
    sleep ; stall RAW                                   ; pc=0x04B8
    sleep ; stall RAW                                   ; pc=0x04BC
    addiHIGH x5, x0, 0                                  ; pc=0x04C0
    sleep ; stall RAW                                   ; pc=0x04C4
    sleep ; stall RAW                                   ; pc=0x04C8
    sleep ; stall RAW                                   ; pc=0x04CC
    addi x5, x5, 32800                                  ; pc=0x04D0
    sleep ; stall RAW                                   ; pc=0x04D4
    sleep ; stall RAW                                   ; pc=0x04D8
    sleep ; stall RAW                                   ; pc=0x04DC
    ; base roundtrip
    add x5, x5, x4                                      ; pc=0x04E0
    sleep ; stall RAW                                   ; pc=0x04E4
    sleep ; stall RAW                                   ; pc=0x04E8
    sleep ; stall RAW                                   ; pc=0x04EC
    lw x4, 0(x5)                                        ; pc=0x04F0
    sleep ; stall RAW                                   ; pc=0x04F4
    sleep ; stall RAW                                   ; pc=0x04F8
    sleep ; stall RAW                                   ; pc=0x04FC
    addi x5, x0, 0                                      ; pc=0x0500
    sleep ; stall RAW                                   ; pc=0x0504
    sleep ; stall RAW                                   ; pc=0x0508
    sleep ; stall RAW                                   ; pc=0x050C
    add x3, x5, x5                                      ; pc=0x0510
    sleep ; stall RAW                                   ; pc=0x0514
    sleep ; stall RAW                                   ; pc=0x0518
    sleep ; stall RAW                                   ; pc=0x051C
    add x3, x3, x3                                      ; pc=0x0520
    sleep ; stall RAW                                   ; pc=0x0524
    sleep ; stall RAW                                   ; pc=0x0528
    sleep ; stall RAW                                   ; pc=0x052C
    addiHIGH x6, x0, 0                                  ; pc=0x0530
    sleep ; stall RAW                                   ; pc=0x0534
    sleep ; stall RAW                                   ; pc=0x0538
    sleep ; stall RAW                                   ; pc=0x053C
    addi x6, x6, 32784                                  ; pc=0x0540
    sleep ; stall RAW                                   ; pc=0x0544
    sleep ; stall RAW                                   ; pc=0x0548
    sleep ; stall RAW                                   ; pc=0x054C
    ; base plain
    add x6, x6, x3                                      ; pc=0x0550
    sleep ; stall RAW                                   ; pc=0x0554
    sleep ; stall RAW                                   ; pc=0x0558
    sleep ; stall RAW                                   ; pc=0x055C
    lw x3, 0(x6)                                        ; pc=0x0560
    sleep ; stall RAW                                   ; pc=0x0564
    sleep ; stall RAW                                   ; pc=0x0568
    sleep ; stall RAW                                   ; pc=0x056C
    bne x4, x3, 316                                     ; pc=0x0570 ; target=.L4_if_else ; addr=0x06AC
    sleep ; nop despues de control                      ; pc=0x0574
    sleep ; nop despues de control                      ; pc=0x0578

    ; if
    addi x3, x0, 1                                      ; pc=0x057C
    sleep ; stall RAW                                   ; pc=0x0580
    sleep ; stall RAW                                   ; pc=0x0584
    sleep ; stall RAW                                   ; pc=0x0588
    add x4, x3, x3                                      ; pc=0x058C
    sleep ; stall RAW                                   ; pc=0x0590
    sleep ; stall RAW                                   ; pc=0x0594
    sleep ; stall RAW                                   ; pc=0x0598
    add x4, x4, x4                                      ; pc=0x059C
    sleep ; stall RAW                                   ; pc=0x05A0
    sleep ; stall RAW                                   ; pc=0x05A4
    sleep ; stall RAW                                   ; pc=0x05A8
    addiHIGH x6, x0, 0                                  ; pc=0x05AC
    sleep ; stall RAW                                   ; pc=0x05B0
    sleep ; stall RAW                                   ; pc=0x05B4
    sleep ; stall RAW                                   ; pc=0x05B8
    addi x6, x6, 32800                                  ; pc=0x05BC
    sleep ; stall RAW                                   ; pc=0x05C0
    sleep ; stall RAW                                   ; pc=0x05C4
    sleep ; stall RAW                                   ; pc=0x05C8
    ; base roundtrip
    add x6, x6, x4                                      ; pc=0x05CC
    sleep ; stall RAW                                   ; pc=0x05D0
    sleep ; stall RAW                                   ; pc=0x05D4
    sleep ; stall RAW                                   ; pc=0x05D8
    lw x4, 0(x6)                                        ; pc=0x05DC
    sleep ; stall RAW                                   ; pc=0x05E0
    sleep ; stall RAW                                   ; pc=0x05E4
    sleep ; stall RAW                                   ; pc=0x05E8
    addi x6, x0, 1                                      ; pc=0x05EC
    sleep ; stall RAW                                   ; pc=0x05F0
    sleep ; stall RAW                                   ; pc=0x05F4
    sleep ; stall RAW                                   ; pc=0x05F8
    add x3, x6, x6                                      ; pc=0x05FC
    sleep ; stall RAW                                   ; pc=0x0600
    sleep ; stall RAW                                   ; pc=0x0604
    sleep ; stall RAW                                   ; pc=0x0608
    add x3, x3, x3                                      ; pc=0x060C
    sleep ; stall RAW                                   ; pc=0x0610
    sleep ; stall RAW                                   ; pc=0x0614
    sleep ; stall RAW                                   ; pc=0x0618
    addiHIGH x5, x0, 0                                  ; pc=0x061C
    sleep ; stall RAW                                   ; pc=0x0620
    sleep ; stall RAW                                   ; pc=0x0624
    sleep ; stall RAW                                   ; pc=0x0628
    addi x5, x5, 32784                                  ; pc=0x062C
    sleep ; stall RAW                                   ; pc=0x0630
    sleep ; stall RAW                                   ; pc=0x0634
    sleep ; stall RAW                                   ; pc=0x0638
    ; base plain
    add x5, x5, x3                                      ; pc=0x063C
    sleep ; stall RAW                                   ; pc=0x0640
    sleep ; stall RAW                                   ; pc=0x0644
    sleep ; stall RAW                                   ; pc=0x0648
    lw x3, 0(x5)                                        ; pc=0x064C
    sleep ; stall RAW                                   ; pc=0x0650
    sleep ; stall RAW                                   ; pc=0x0654
    sleep ; stall RAW                                   ; pc=0x0658
    bne x4, x3, 68                                      ; pc=0x065C ; target=.L6_if_else ; addr=0x06A0
    sleep ; nop despues de control                      ; pc=0x0660
    sleep ; nop despues de control                      ; pc=0x0664
    addi x3, x0, 0                                      ; pc=0x0668
    sleep ; stall RAW                                   ; pc=0x066C
    sleep ; stall RAW                                   ; pc=0x0670
    sleep ; stall RAW                                   ; pc=0x0674
    add x11, x3, x0                                     ; pc=0x0678
    sleep ; stall RAW                                   ; pc=0x067C
    sleep ; stall RAW                                   ; pc=0x0680
    sleep ; stall RAW                                   ; pc=0x0684
    jal x0, 80                                          ; pc=0x0688 ; target=.L_codegen_1_main_end ; addr=0x06D8
    sleep ; nop despues de control                      ; pc=0x068C
    sleep ; nop despues de control                      ; pc=0x0690
    jal x0, 12                                          ; pc=0x0694 ; target=.L7_if_end ; addr=0x06A0
    sleep ; nop despues de control                      ; pc=0x0698
    sleep ; nop despues de control                      ; pc=0x069C
.L6_if_else:
.L7_if_end:

    jal x0, 12                                          ; pc=0x06A0 ; target=.L5_if_end ; addr=0x06AC
    sleep ; nop despues de control                      ; pc=0x06A4
    sleep ; nop despues de control                      ; pc=0x06A8
.L4_if_else:
.L5_if_end:

    addi x3, x0, 1                                      ; pc=0x06AC
    sleep ; stall RAW                                   ; pc=0x06B0
    sleep ; stall RAW                                   ; pc=0x06B4
    sleep ; stall RAW                                   ; pc=0x06B8
    add x11, x3, x0                                     ; pc=0x06BC
    sleep ; stall RAW                                   ; pc=0x06C0
    sleep ; stall RAW                                   ; pc=0x06C4
    sleep ; stall RAW                                   ; pc=0x06C8
    jal x0, 12                                          ; pc=0x06CC ; target=.L_codegen_1_main_end ; addr=0x06D8
    sleep ; nop despues de control                      ; pc=0x06D0
    sleep ; nop despues de control                      ; pc=0x06D4
.L_codegen_1_main_end:
    ; epilogue
    lw x17, 4(x2)                                       ; pc=0x06D8
    sleep ; stall RAW                                   ; pc=0x06DC
    sleep ; stall RAW                                   ; pc=0x06E0
    sleep ; stall RAW                                   ; pc=0x06E4
    addi x2, x2, 8                                      ; pc=0x06E8
    sleep ; stall RAW                                   ; pc=0x06EC
    sleep ; stall RAW                                   ; pc=0x06F0
    sleep ; stall RAW                                   ; pc=0x06F4
    freeze                                              ; pc=0x06F8

tea_encrypt:
    ; prologue
    addiSigned x2, x2, -60                              ; pc=0x06FC
    sleep ; stall RAW                                   ; pc=0x0700
    sleep ; stall RAW                                   ; pc=0x0704
    sleep ; stall RAW                                   ; pc=0x0708
    sw x1, 0(x2)                                        ; pc=0x070C
    sw x17, 4(x2)                                       ; pc=0x0710
    addi x17, x2, 60                                    ; pc=0x0714
    sleep ; stall RAW                                   ; pc=0x0718
    sleep ; stall RAW                                   ; pc=0x071C
    sleep ; stall RAW                                   ; pc=0x0720

    sw x11, -4(x17) ; parámetro v                       ; pc=0x0724
    sw x12, -8(x17) ; parámetro tea_key                 ; pc=0x0728

    addi x3, x0, 0                                      ; pc=0x072C
    sleep ; stall RAW                                   ; pc=0x0730
    sleep ; stall RAW                                   ; pc=0x0734
    sleep ; stall RAW                                   ; pc=0x0738
    add x4, x3, x3                                      ; pc=0x073C
    sleep ; stall RAW                                   ; pc=0x0740
    sleep ; stall RAW                                   ; pc=0x0744
    sleep ; stall RAW                                   ; pc=0x0748
    add x4, x4, x4                                      ; pc=0x074C
    sleep ; stall RAW                                   ; pc=0x0750
    sleep ; stall RAW                                   ; pc=0x0754
    sleep ; stall RAW                                   ; pc=0x0758
    lw x5, -4(x17) ; base ref v                         ; pc=0x075C
    sleep ; stall RAW                                   ; pc=0x0760
    sleep ; stall RAW                                   ; pc=0x0764
    sleep ; stall RAW                                   ; pc=0x0768
    add x5, x5, x4                                      ; pc=0x076C
    sleep ; stall RAW                                   ; pc=0x0770
    sleep ; stall RAW                                   ; pc=0x0774
    sleep ; stall RAW                                   ; pc=0x0778
    lw x4, 0(x5)                                        ; pc=0x077C
    sleep ; stall RAW                                   ; pc=0x0780
    sleep ; stall RAW                                   ; pc=0x0784
    sleep ; stall RAW                                   ; pc=0x0788
    sw x4, -12(x17) ; v0                                ; pc=0x078C
    addi x4, x0, 1                                      ; pc=0x0790
    sleep ; stall RAW                                   ; pc=0x0794
    sleep ; stall RAW                                   ; pc=0x0798
    sleep ; stall RAW                                   ; pc=0x079C
    add x5, x4, x4                                      ; pc=0x07A0
    sleep ; stall RAW                                   ; pc=0x07A4
    sleep ; stall RAW                                   ; pc=0x07A8
    sleep ; stall RAW                                   ; pc=0x07AC
    add x5, x5, x5                                      ; pc=0x07B0
    sleep ; stall RAW                                   ; pc=0x07B4
    sleep ; stall RAW                                   ; pc=0x07B8
    sleep ; stall RAW                                   ; pc=0x07BC
    lw x3, -4(x17) ; base ref v                         ; pc=0x07C0
    sleep ; stall RAW                                   ; pc=0x07C4
    sleep ; stall RAW                                   ; pc=0x07C8
    sleep ; stall RAW                                   ; pc=0x07CC
    add x3, x3, x5                                      ; pc=0x07D0
    sleep ; stall RAW                                   ; pc=0x07D4
    sleep ; stall RAW                                   ; pc=0x07D8
    sleep ; stall RAW                                   ; pc=0x07DC
    lw x5, 0(x3)                                        ; pc=0x07E0
    sleep ; stall RAW                                   ; pc=0x07E4
    sleep ; stall RAW                                   ; pc=0x07E8
    sleep ; stall RAW                                   ; pc=0x07EC
    sw x5, -16(x17) ; v1                                ; pc=0x07F0
    addi x5, x0, 0                                      ; pc=0x07F4
    sleep ; stall RAW                                   ; pc=0x07F8
    sleep ; stall RAW                                   ; pc=0x07FC
    sleep ; stall RAW                                   ; pc=0x0800
    sw x5, -20(x17) ; sum                               ; pc=0x0804

    ; for
    addi x5, x0, 0                                      ; pc=0x0808
    sleep ; stall RAW                                   ; pc=0x080C
    sleep ; stall RAW                                   ; pc=0x0810
    sleep ; stall RAW                                   ; pc=0x0814
    sw x5, -24(x17) ; i                                 ; pc=0x0818
.L0_for_start:
    lw x5, -24(x17) ; i                                 ; pc=0x081C
    sleep ; stall RAW                                   ; pc=0x0820
    sleep ; stall RAW                                   ; pc=0x0824
    sleep ; stall RAW                                   ; pc=0x0828
    addi x3, x0, 32                                     ; pc=0x082C
    sleep ; stall RAW                                   ; pc=0x0830
    sleep ; stall RAW                                   ; pc=0x0834
    sleep ; stall RAW                                   ; pc=0x0838
    bge x5, x3, 1152                                    ; pc=0x083C ; target=.L1_for_end ; addr=0x0CBC
    sleep ; nop despues de control                      ; pc=0x0840
    sleep ; nop despues de control                      ; pc=0x0844
    lw x3, -20(x17) ; sum                               ; pc=0x0848
    sleep ; stall RAW                                   ; pc=0x084C
    sleep ; stall RAW                                   ; pc=0x0850
    sleep ; stall RAW                                   ; pc=0x0854
    addiHIGH x4, x0, 0                                  ; pc=0x0858
    sleep ; stall RAW                                   ; pc=0x085C
    sleep ; stall RAW                                   ; pc=0x0860
    sleep ; stall RAW                                   ; pc=0x0864
    addi x4, x4, 32808                                  ; pc=0x0868
    sleep ; stall RAW                                   ; pc=0x086C
    sleep ; stall RAW                                   ; pc=0x0870
    sleep ; stall RAW                                   ; pc=0x0874
    lw x5, 0(x4) ; DELTA                                ; pc=0x0878
    sleep ; stall RAW                                   ; pc=0x087C
    sleep ; stall RAW                                   ; pc=0x0880
    sleep ; stall RAW                                   ; pc=0x0884
    add x4, x3, x5                                      ; pc=0x0888
    sleep ; stall RAW                                   ; pc=0x088C
    sleep ; stall RAW                                   ; pc=0x0890
    sleep ; stall RAW                                   ; pc=0x0894
    sw x4, -20(x17) ; sum                               ; pc=0x0898
    lw x4, -16(x17) ; v1                                ; pc=0x089C
    sleep ; stall RAW                                   ; pc=0x08A0
    sleep ; stall RAW                                   ; pc=0x08A4
    sleep ; stall RAW                                   ; pc=0x08A8
    addi x5, x0, 0                                      ; pc=0x08AC
    sleep ; stall RAW                                   ; pc=0x08B0
    sleep ; stall RAW                                   ; pc=0x08B4
    sleep ; stall RAW                                   ; pc=0x08B8
    add x3, x5, x5                                      ; pc=0x08BC
    sleep ; stall RAW                                   ; pc=0x08C0
    sleep ; stall RAW                                   ; pc=0x08C4
    sleep ; stall RAW                                   ; pc=0x08C8
    add x3, x3, x3                                      ; pc=0x08CC
    sleep ; stall RAW                                   ; pc=0x08D0
    sleep ; stall RAW                                   ; pc=0x08D4
    sleep ; stall RAW                                   ; pc=0x08D8
    lw x6, -8(x17) ; base ref tea_key                   ; pc=0x08DC
    sleep ; stall RAW                                   ; pc=0x08E0
    sleep ; stall RAW                                   ; pc=0x08E4
    sleep ; stall RAW                                   ; pc=0x08E8
    add x6, x6, x3                                      ; pc=0x08EC
    sleep ; stall RAW                                   ; pc=0x08F0
    sleep ; stall RAW                                   ; pc=0x08F4
    sleep ; stall RAW                                   ; pc=0x08F8
    lw x3, 0(x6)                                        ; pc=0x08FC
    sleep ; stall RAW                                   ; pc=0x0900
    sleep ; stall RAW                                   ; pc=0x0904
    sleep ; stall RAW                                   ; pc=0x0908
    addi x5, x0, 4                                      ; pc=0x090C
    sleep ; stall RAW                                   ; pc=0x0910
    sleep ; stall RAW                                   ; pc=0x0914
    sleep ; stall RAW                                   ; pc=0x0918
    sll x6, x4, x5                                      ; pc=0x091C
    sleep ; stall RAW                                   ; pc=0x0920
    sleep ; stall RAW                                   ; pc=0x0924
    sleep ; stall RAW                                   ; pc=0x0928
    add x6, x6, x3                                      ; pc=0x092C
    sleep ; stall RAW                                   ; pc=0x0930
    sleep ; stall RAW                                   ; pc=0x0934
    sleep ; stall RAW                                   ; pc=0x0938
    sw x6, -28(x17) ; left0                             ; pc=0x093C
    lw x6, -16(x17) ; v1                                ; pc=0x0940
    sleep ; stall RAW                                   ; pc=0x0944
    sleep ; stall RAW                                   ; pc=0x0948
    sleep ; stall RAW                                   ; pc=0x094C
    lw x3, -20(x17) ; sum                               ; pc=0x0950
    sleep ; stall RAW                                   ; pc=0x0954
    sleep ; stall RAW                                   ; pc=0x0958
    sleep ; stall RAW                                   ; pc=0x095C
    add x4, x6, x3                                      ; pc=0x0960
    sleep ; stall RAW                                   ; pc=0x0964
    sleep ; stall RAW                                   ; pc=0x0968
    sleep ; stall RAW                                   ; pc=0x096C
    sw x4, -32(x17) ; mid0                              ; pc=0x0970
    lw x4, -16(x17) ; v1                                ; pc=0x0974
    sleep ; stall RAW                                   ; pc=0x0978
    sleep ; stall RAW                                   ; pc=0x097C
    sleep ; stall RAW                                   ; pc=0x0980
    addi x3, x0, 1                                      ; pc=0x0984
    sleep ; stall RAW                                   ; pc=0x0988
    sleep ; stall RAW                                   ; pc=0x098C
    sleep ; stall RAW                                   ; pc=0x0990
    add x6, x3, x3                                      ; pc=0x0994
    sleep ; stall RAW                                   ; pc=0x0998
    sleep ; stall RAW                                   ; pc=0x099C
    sleep ; stall RAW                                   ; pc=0x09A0
    add x6, x6, x6                                      ; pc=0x09A4
    sleep ; stall RAW                                   ; pc=0x09A8
    sleep ; stall RAW                                   ; pc=0x09AC
    sleep ; stall RAW                                   ; pc=0x09B0
    lw x5, -8(x17) ; base ref tea_key                   ; pc=0x09B4
    sleep ; stall RAW                                   ; pc=0x09B8
    sleep ; stall RAW                                   ; pc=0x09BC
    sleep ; stall RAW                                   ; pc=0x09C0
    add x5, x5, x6                                      ; pc=0x09C4
    sleep ; stall RAW                                   ; pc=0x09C8
    sleep ; stall RAW                                   ; pc=0x09CC
    sleep ; stall RAW                                   ; pc=0x09D0
    lw x6, 0(x5)                                        ; pc=0x09D4
    sleep ; stall RAW                                   ; pc=0x09D8
    sleep ; stall RAW                                   ; pc=0x09DC
    sleep ; stall RAW                                   ; pc=0x09E0
    addi x3, x0, 5                                      ; pc=0x09E4
    sleep ; stall RAW                                   ; pc=0x09E8
    sleep ; stall RAW                                   ; pc=0x09EC
    sleep ; stall RAW                                   ; pc=0x09F0
    srl x5, x4, x3                                      ; pc=0x09F4
    sleep ; stall RAW                                   ; pc=0x09F8
    sleep ; stall RAW                                   ; pc=0x09FC
    sleep ; stall RAW                                   ; pc=0x0A00
    add x5, x5, x6                                      ; pc=0x0A04
    sleep ; stall RAW                                   ; pc=0x0A08
    sleep ; stall RAW                                   ; pc=0x0A0C
    sleep ; stall RAW                                   ; pc=0x0A10
    sw x5, -36(x17) ; right0                            ; pc=0x0A14
    lw x5, -12(x17) ; v0                                ; pc=0x0A18
    sleep ; stall RAW                                   ; pc=0x0A1C
    sleep ; stall RAW                                   ; pc=0x0A20
    sleep ; stall RAW                                   ; pc=0x0A24
    lw x6, -28(x17) ; left0                             ; pc=0x0A28
    sleep ; stall RAW                                   ; pc=0x0A2C
    sleep ; stall RAW                                   ; pc=0x0A30
    sleep ; stall RAW                                   ; pc=0x0A34
    lw x4, -32(x17) ; mid0                              ; pc=0x0A38
    sleep ; stall RAW                                   ; pc=0x0A3C
    sleep ; stall RAW                                   ; pc=0x0A40
    sleep ; stall RAW                                   ; pc=0x0A44
    xor x3, x6, x4                                      ; pc=0x0A48
    sleep ; stall RAW                                   ; pc=0x0A4C
    sleep ; stall RAW                                   ; pc=0x0A50
    sleep ; stall RAW                                   ; pc=0x0A54
    lw x4, -36(x17) ; right0                            ; pc=0x0A58
    sleep ; stall RAW                                   ; pc=0x0A5C
    sleep ; stall RAW                                   ; pc=0x0A60
    sleep ; stall RAW                                   ; pc=0x0A64
    xor x6, x3, x4                                      ; pc=0x0A68
    sleep ; stall RAW                                   ; pc=0x0A6C
    sleep ; stall RAW                                   ; pc=0x0A70
    sleep ; stall RAW                                   ; pc=0x0A74
    add x4, x5, x6                                      ; pc=0x0A78
    sleep ; stall RAW                                   ; pc=0x0A7C
    sleep ; stall RAW                                   ; pc=0x0A80
    sleep ; stall RAW                                   ; pc=0x0A84
    sw x4, -12(x17) ; v0                                ; pc=0x0A88
    lw x4, -12(x17) ; v0                                ; pc=0x0A8C
    sleep ; stall RAW                                   ; pc=0x0A90
    sleep ; stall RAW                                   ; pc=0x0A94
    sleep ; stall RAW                                   ; pc=0x0A98
    addi x6, x0, 2                                      ; pc=0x0A9C
    sleep ; stall RAW                                   ; pc=0x0AA0
    sleep ; stall RAW                                   ; pc=0x0AA4
    sleep ; stall RAW                                   ; pc=0x0AA8
    add x5, x6, x6                                      ; pc=0x0AAC
    sleep ; stall RAW                                   ; pc=0x0AB0
    sleep ; stall RAW                                   ; pc=0x0AB4
    sleep ; stall RAW                                   ; pc=0x0AB8
    add x5, x5, x5                                      ; pc=0x0ABC
    sleep ; stall RAW                                   ; pc=0x0AC0
    sleep ; stall RAW                                   ; pc=0x0AC4
    sleep ; stall RAW                                   ; pc=0x0AC8
    lw x3, -8(x17) ; base ref tea_key                   ; pc=0x0ACC
    sleep ; stall RAW                                   ; pc=0x0AD0
    sleep ; stall RAW                                   ; pc=0x0AD4
    sleep ; stall RAW                                   ; pc=0x0AD8
    add x3, x3, x5                                      ; pc=0x0ADC
    sleep ; stall RAW                                   ; pc=0x0AE0
    sleep ; stall RAW                                   ; pc=0x0AE4
    sleep ; stall RAW                                   ; pc=0x0AE8
    lw x5, 0(x3)                                        ; pc=0x0AEC
    sleep ; stall RAW                                   ; pc=0x0AF0
    sleep ; stall RAW                                   ; pc=0x0AF4
    sleep ; stall RAW                                   ; pc=0x0AF8
    addi x6, x0, 4                                      ; pc=0x0AFC
    sleep ; stall RAW                                   ; pc=0x0B00
    sleep ; stall RAW                                   ; pc=0x0B04
    sleep ; stall RAW                                   ; pc=0x0B08
    sll x3, x4, x6                                      ; pc=0x0B0C
    sleep ; stall RAW                                   ; pc=0x0B10
    sleep ; stall RAW                                   ; pc=0x0B14
    sleep ; stall RAW                                   ; pc=0x0B18
    add x3, x3, x5                                      ; pc=0x0B1C
    sleep ; stall RAW                                   ; pc=0x0B20
    sleep ; stall RAW                                   ; pc=0x0B24
    sleep ; stall RAW                                   ; pc=0x0B28
    sw x3, -40(x17) ; left1                             ; pc=0x0B2C
    lw x3, -12(x17) ; v0                                ; pc=0x0B30
    sleep ; stall RAW                                   ; pc=0x0B34
    sleep ; stall RAW                                   ; pc=0x0B38
    sleep ; stall RAW                                   ; pc=0x0B3C
    lw x5, -20(x17) ; sum                               ; pc=0x0B40
    sleep ; stall RAW                                   ; pc=0x0B44
    sleep ; stall RAW                                   ; pc=0x0B48
    sleep ; stall RAW                                   ; pc=0x0B4C
    add x4, x3, x5                                      ; pc=0x0B50
    sleep ; stall RAW                                   ; pc=0x0B54
    sleep ; stall RAW                                   ; pc=0x0B58
    sleep ; stall RAW                                   ; pc=0x0B5C
    sw x4, -44(x17) ; mid1                              ; pc=0x0B60
    lw x4, -12(x17) ; v0                                ; pc=0x0B64
    sleep ; stall RAW                                   ; pc=0x0B68
    sleep ; stall RAW                                   ; pc=0x0B6C
    sleep ; stall RAW                                   ; pc=0x0B70
    addi x5, x0, 3                                      ; pc=0x0B74
    sleep ; stall RAW                                   ; pc=0x0B78
    sleep ; stall RAW                                   ; pc=0x0B7C
    sleep ; stall RAW                                   ; pc=0x0B80
    add x3, x5, x5                                      ; pc=0x0B84
    sleep ; stall RAW                                   ; pc=0x0B88
    sleep ; stall RAW                                   ; pc=0x0B8C
    sleep ; stall RAW                                   ; pc=0x0B90
    add x3, x3, x3                                      ; pc=0x0B94
    sleep ; stall RAW                                   ; pc=0x0B98
    sleep ; stall RAW                                   ; pc=0x0B9C
    sleep ; stall RAW                                   ; pc=0x0BA0
    lw x6, -8(x17) ; base ref tea_key                   ; pc=0x0BA4
    sleep ; stall RAW                                   ; pc=0x0BA8
    sleep ; stall RAW                                   ; pc=0x0BAC
    sleep ; stall RAW                                   ; pc=0x0BB0
    add x6, x6, x3                                      ; pc=0x0BB4
    sleep ; stall RAW                                   ; pc=0x0BB8
    sleep ; stall RAW                                   ; pc=0x0BBC
    sleep ; stall RAW                                   ; pc=0x0BC0
    lw x3, 0(x6)                                        ; pc=0x0BC4
    sleep ; stall RAW                                   ; pc=0x0BC8
    sleep ; stall RAW                                   ; pc=0x0BCC
    sleep ; stall RAW                                   ; pc=0x0BD0
    addi x5, x0, 5                                      ; pc=0x0BD4
    sleep ; stall RAW                                   ; pc=0x0BD8
    sleep ; stall RAW                                   ; pc=0x0BDC
    sleep ; stall RAW                                   ; pc=0x0BE0
    srl x6, x4, x5                                      ; pc=0x0BE4
    sleep ; stall RAW                                   ; pc=0x0BE8
    sleep ; stall RAW                                   ; pc=0x0BEC
    sleep ; stall RAW                                   ; pc=0x0BF0
    add x6, x6, x3                                      ; pc=0x0BF4
    sleep ; stall RAW                                   ; pc=0x0BF8
    sleep ; stall RAW                                   ; pc=0x0BFC
    sleep ; stall RAW                                   ; pc=0x0C00
    sw x6, -48(x17) ; right1                            ; pc=0x0C04
    lw x6, -16(x17) ; v1                                ; pc=0x0C08
    sleep ; stall RAW                                   ; pc=0x0C0C
    sleep ; stall RAW                                   ; pc=0x0C10
    sleep ; stall RAW                                   ; pc=0x0C14
    lw x3, -40(x17) ; left1                             ; pc=0x0C18
    sleep ; stall RAW                                   ; pc=0x0C1C
    sleep ; stall RAW                                   ; pc=0x0C20
    sleep ; stall RAW                                   ; pc=0x0C24
    lw x4, -44(x17) ; mid1                              ; pc=0x0C28
    sleep ; stall RAW                                   ; pc=0x0C2C
    sleep ; stall RAW                                   ; pc=0x0C30
    sleep ; stall RAW                                   ; pc=0x0C34
    xor x5, x3, x4                                      ; pc=0x0C38
    sleep ; stall RAW                                   ; pc=0x0C3C
    sleep ; stall RAW                                   ; pc=0x0C40
    sleep ; stall RAW                                   ; pc=0x0C44
    lw x4, -48(x17) ; right1                            ; pc=0x0C48
    sleep ; stall RAW                                   ; pc=0x0C4C
    sleep ; stall RAW                                   ; pc=0x0C50
    sleep ; stall RAW                                   ; pc=0x0C54
    xor x3, x5, x4                                      ; pc=0x0C58
    sleep ; stall RAW                                   ; pc=0x0C5C
    sleep ; stall RAW                                   ; pc=0x0C60
    sleep ; stall RAW                                   ; pc=0x0C64
    add x4, x6, x3                                      ; pc=0x0C68
    sleep ; stall RAW                                   ; pc=0x0C6C
    sleep ; stall RAW                                   ; pc=0x0C70
    sleep ; stall RAW                                   ; pc=0x0C74
    sw x4, -16(x17) ; v1                                ; pc=0x0C78
    lw x4, -24(x17) ; i                                 ; pc=0x0C7C
    sleep ; stall RAW                                   ; pc=0x0C80
    sleep ; stall RAW                                   ; pc=0x0C84
    sleep ; stall RAW                                   ; pc=0x0C88
    addi x3, x0, 1                                      ; pc=0x0C8C
    sleep ; stall RAW                                   ; pc=0x0C90
    sleep ; stall RAW                                   ; pc=0x0C94
    sleep ; stall RAW                                   ; pc=0x0C98
    add x6, x4, x3                                      ; pc=0x0C9C
    sleep ; stall RAW                                   ; pc=0x0CA0
    sleep ; stall RAW                                   ; pc=0x0CA4
    sleep ; stall RAW                                   ; pc=0x0CA8
    sw x6, -24(x17) ; i                                 ; pc=0x0CAC
    jal x0, -1172                                       ; pc=0x0CB0 ; target=.L0_for_start ; addr=0x081C
    sleep ; nop despues de control                      ; pc=0x0CB4
    sleep ; nop despues de control                      ; pc=0x0CB8
.L1_for_end:

    lw x6, -12(x17) ; v0                                ; pc=0x0CBC
    sleep ; stall RAW                                   ; pc=0x0CC0
    sleep ; stall RAW                                   ; pc=0x0CC4
    sleep ; stall RAW                                   ; pc=0x0CC8
    addi x3, x0, 0                                      ; pc=0x0CCC
    sleep ; stall RAW                                   ; pc=0x0CD0
    sleep ; stall RAW                                   ; pc=0x0CD4
    sleep ; stall RAW                                   ; pc=0x0CD8
    add x4, x3, x3                                      ; pc=0x0CDC
    sleep ; stall RAW                                   ; pc=0x0CE0
    sleep ; stall RAW                                   ; pc=0x0CE4
    sleep ; stall RAW                                   ; pc=0x0CE8
    add x4, x4, x4                                      ; pc=0x0CEC
    sleep ; stall RAW                                   ; pc=0x0CF0
    sleep ; stall RAW                                   ; pc=0x0CF4
    sleep ; stall RAW                                   ; pc=0x0CF8
    lw x5, -4(x17) ; base ref v                         ; pc=0x0CFC
    sleep ; stall RAW                                   ; pc=0x0D00
    sleep ; stall RAW                                   ; pc=0x0D04
    sleep ; stall RAW                                   ; pc=0x0D08
    add x5, x5, x4                                      ; pc=0x0D0C
    sleep ; stall RAW                                   ; pc=0x0D10
    sleep ; stall RAW                                   ; pc=0x0D14
    sleep ; stall RAW                                   ; pc=0x0D18
    sw x6, 0(x5)                                        ; pc=0x0D1C
    lw x6, -16(x17) ; v1                                ; pc=0x0D20
    sleep ; stall RAW                                   ; pc=0x0D24
    sleep ; stall RAW                                   ; pc=0x0D28
    sleep ; stall RAW                                   ; pc=0x0D2C
    addi x5, x0, 1                                      ; pc=0x0D30
    sleep ; stall RAW                                   ; pc=0x0D34
    sleep ; stall RAW                                   ; pc=0x0D38
    sleep ; stall RAW                                   ; pc=0x0D3C
    add x4, x5, x5                                      ; pc=0x0D40
    sleep ; stall RAW                                   ; pc=0x0D44
    sleep ; stall RAW                                   ; pc=0x0D48
    sleep ; stall RAW                                   ; pc=0x0D4C
    add x4, x4, x4                                      ; pc=0x0D50
    sleep ; stall RAW                                   ; pc=0x0D54
    sleep ; stall RAW                                   ; pc=0x0D58
    sleep ; stall RAW                                   ; pc=0x0D5C
    lw x3, -4(x17) ; base ref v                         ; pc=0x0D60
    sleep ; stall RAW                                   ; pc=0x0D64
    sleep ; stall RAW                                   ; pc=0x0D68
    sleep ; stall RAW                                   ; pc=0x0D6C
    add x3, x3, x4                                      ; pc=0x0D70
    sleep ; stall RAW                                   ; pc=0x0D74
    sleep ; stall RAW                                   ; pc=0x0D78
    sleep ; stall RAW                                   ; pc=0x0D7C
    sw x6, 0(x3)                                        ; pc=0x0D80
.L_codegen_2_tea_encrypt_end:
    ; epilogue
    lw x1, 0(x2)                                        ; pc=0x0D84
    sleep ; stall RAW                                   ; pc=0x0D88
    sleep ; stall RAW                                   ; pc=0x0D8C
    sleep ; stall RAW                                   ; pc=0x0D90
    lw x17, 4(x2)                                       ; pc=0x0D94
    sleep ; stall RAW                                   ; pc=0x0D98
    sleep ; stall RAW                                   ; pc=0x0D9C
    sleep ; stall RAW                                   ; pc=0x0DA0
    addi x2, x2, 60                                     ; pc=0x0DA4
    sleep ; stall RAW                                   ; pc=0x0DA8
    sleep ; stall RAW                                   ; pc=0x0DAC
    sleep ; stall RAW                                   ; pc=0x0DB0
    jalr x1, 0                                          ; pc=0x0DB4
    sleep ; nop despues de control                      ; pc=0x0DB8
    sleep ; nop despues de control                      ; pc=0x0DBC

tea_decrypt:
    ; prologue
    addiSigned x2, x2, -60                              ; pc=0x0DC0
    sleep ; stall RAW                                   ; pc=0x0DC4
    sleep ; stall RAW                                   ; pc=0x0DC8
    sleep ; stall RAW                                   ; pc=0x0DCC
    sw x1, 0(x2)                                        ; pc=0x0DD0
    sw x17, 4(x2)                                       ; pc=0x0DD4
    addi x17, x2, 60                                    ; pc=0x0DD8
    sleep ; stall RAW                                   ; pc=0x0DDC
    sleep ; stall RAW                                   ; pc=0x0DE0
    sleep ; stall RAW                                   ; pc=0x0DE4

    sw x11, -4(x17) ; parámetro v                       ; pc=0x0DE8
    sw x12, -8(x17) ; parámetro tea_key                 ; pc=0x0DEC

    addi x6, x0, 0                                      ; pc=0x0DF0
    sleep ; stall RAW                                   ; pc=0x0DF4
    sleep ; stall RAW                                   ; pc=0x0DF8
    sleep ; stall RAW                                   ; pc=0x0DFC
    add x3, x6, x6                                      ; pc=0x0E00
    sleep ; stall RAW                                   ; pc=0x0E04
    sleep ; stall RAW                                   ; pc=0x0E08
    sleep ; stall RAW                                   ; pc=0x0E0C
    add x3, x3, x3                                      ; pc=0x0E10
    sleep ; stall RAW                                   ; pc=0x0E14
    sleep ; stall RAW                                   ; pc=0x0E18
    sleep ; stall RAW                                   ; pc=0x0E1C
    lw x4, -4(x17) ; base ref v                         ; pc=0x0E20
    sleep ; stall RAW                                   ; pc=0x0E24
    sleep ; stall RAW                                   ; pc=0x0E28
    sleep ; stall RAW                                   ; pc=0x0E2C
    add x4, x4, x3                                      ; pc=0x0E30
    sleep ; stall RAW                                   ; pc=0x0E34
    sleep ; stall RAW                                   ; pc=0x0E38
    sleep ; stall RAW                                   ; pc=0x0E3C
    lw x3, 0(x4)                                        ; pc=0x0E40
    sleep ; stall RAW                                   ; pc=0x0E44
    sleep ; stall RAW                                   ; pc=0x0E48
    sleep ; stall RAW                                   ; pc=0x0E4C
    sw x3, -12(x17) ; v0                                ; pc=0x0E50
    addi x3, x0, 1                                      ; pc=0x0E54
    sleep ; stall RAW                                   ; pc=0x0E58
    sleep ; stall RAW                                   ; pc=0x0E5C
    sleep ; stall RAW                                   ; pc=0x0E60
    add x4, x3, x3                                      ; pc=0x0E64
    sleep ; stall RAW                                   ; pc=0x0E68
    sleep ; stall RAW                                   ; pc=0x0E6C
    sleep ; stall RAW                                   ; pc=0x0E70
    add x4, x4, x4                                      ; pc=0x0E74
    sleep ; stall RAW                                   ; pc=0x0E78
    sleep ; stall RAW                                   ; pc=0x0E7C
    sleep ; stall RAW                                   ; pc=0x0E80
    lw x6, -4(x17) ; base ref v                         ; pc=0x0E84
    sleep ; stall RAW                                   ; pc=0x0E88
    sleep ; stall RAW                                   ; pc=0x0E8C
    sleep ; stall RAW                                   ; pc=0x0E90
    add x6, x6, x4                                      ; pc=0x0E94
    sleep ; stall RAW                                   ; pc=0x0E98
    sleep ; stall RAW                                   ; pc=0x0E9C
    sleep ; stall RAW                                   ; pc=0x0EA0
    lw x4, 0(x6)                                        ; pc=0x0EA4
    sleep ; stall RAW                                   ; pc=0x0EA8
    sleep ; stall RAW                                   ; pc=0x0EAC
    sleep ; stall RAW                                   ; pc=0x0EB0
    sw x4, -16(x17) ; v1                                ; pc=0x0EB4
    addiHIGH x6, x0, 0                                  ; pc=0x0EB8
    sleep ; stall RAW                                   ; pc=0x0EBC
    sleep ; stall RAW                                   ; pc=0x0EC0
    sleep ; stall RAW                                   ; pc=0x0EC4
    addi x6, x6, 32812                                  ; pc=0x0EC8
    sleep ; stall RAW                                   ; pc=0x0ECC
    sleep ; stall RAW                                   ; pc=0x0ED0
    sleep ; stall RAW                                   ; pc=0x0ED4
    lw x4, 0(x6) ; SUM_INIT                             ; pc=0x0ED8
    sleep ; stall RAW                                   ; pc=0x0EDC
    sleep ; stall RAW                                   ; pc=0x0EE0
    sleep ; stall RAW                                   ; pc=0x0EE4
    sw x4, -20(x17) ; sum                               ; pc=0x0EE8

    ; for
    addi x4, x0, 0                                      ; pc=0x0EEC
    sleep ; stall RAW                                   ; pc=0x0EF0
    sleep ; stall RAW                                   ; pc=0x0EF4
    sleep ; stall RAW                                   ; pc=0x0EF8
    sw x4, -24(x17) ; i                                 ; pc=0x0EFC
.L2_for_start:
    lw x4, -24(x17) ; i                                 ; pc=0x0F00
    sleep ; stall RAW                                   ; pc=0x0F04
    sleep ; stall RAW                                   ; pc=0x0F08
    sleep ; stall RAW                                   ; pc=0x0F0C
    addi x6, x0, 32                                     ; pc=0x0F10
    sleep ; stall RAW                                   ; pc=0x0F14
    sleep ; stall RAW                                   ; pc=0x0F18
    sleep ; stall RAW                                   ; pc=0x0F1C
    bge x4, x6, 1152                                    ; pc=0x0F20 ; target=.L3_for_end ; addr=0x13A0
    sleep ; nop despues de control                      ; pc=0x0F24
    sleep ; nop despues de control                      ; pc=0x0F28
    lw x6, -12(x17) ; v0                                ; pc=0x0F2C
    sleep ; stall RAW                                   ; pc=0x0F30
    sleep ; stall RAW                                   ; pc=0x0F34
    sleep ; stall RAW                                   ; pc=0x0F38
    addi x4, x0, 2                                      ; pc=0x0F3C
    sleep ; stall RAW                                   ; pc=0x0F40
    sleep ; stall RAW                                   ; pc=0x0F44
    sleep ; stall RAW                                   ; pc=0x0F48
    add x3, x4, x4                                      ; pc=0x0F4C
    sleep ; stall RAW                                   ; pc=0x0F50
    sleep ; stall RAW                                   ; pc=0x0F54
    sleep ; stall RAW                                   ; pc=0x0F58
    add x3, x3, x3                                      ; pc=0x0F5C
    sleep ; stall RAW                                   ; pc=0x0F60
    sleep ; stall RAW                                   ; pc=0x0F64
    sleep ; stall RAW                                   ; pc=0x0F68
    lw x5, -8(x17) ; base ref tea_key                   ; pc=0x0F6C
    sleep ; stall RAW                                   ; pc=0x0F70
    sleep ; stall RAW                                   ; pc=0x0F74
    sleep ; stall RAW                                   ; pc=0x0F78
    add x5, x5, x3                                      ; pc=0x0F7C
    sleep ; stall RAW                                   ; pc=0x0F80
    sleep ; stall RAW                                   ; pc=0x0F84
    sleep ; stall RAW                                   ; pc=0x0F88
    lw x3, 0(x5)                                        ; pc=0x0F8C
    sleep ; stall RAW                                   ; pc=0x0F90
    sleep ; stall RAW                                   ; pc=0x0F94
    sleep ; stall RAW                                   ; pc=0x0F98
    addi x4, x0, 4                                      ; pc=0x0F9C
    sleep ; stall RAW                                   ; pc=0x0FA0
    sleep ; stall RAW                                   ; pc=0x0FA4
    sleep ; stall RAW                                   ; pc=0x0FA8
    sll x5, x6, x4                                      ; pc=0x0FAC
    sleep ; stall RAW                                   ; pc=0x0FB0
    sleep ; stall RAW                                   ; pc=0x0FB4
    sleep ; stall RAW                                   ; pc=0x0FB8
    add x5, x5, x3                                      ; pc=0x0FBC
    sleep ; stall RAW                                   ; pc=0x0FC0
    sleep ; stall RAW                                   ; pc=0x0FC4
    sleep ; stall RAW                                   ; pc=0x0FC8
    sw x5, -28(x17) ; left1                             ; pc=0x0FCC
    lw x5, -12(x17) ; v0                                ; pc=0x0FD0
    sleep ; stall RAW                                   ; pc=0x0FD4
    sleep ; stall RAW                                   ; pc=0x0FD8
    sleep ; stall RAW                                   ; pc=0x0FDC
    lw x3, -20(x17) ; sum                               ; pc=0x0FE0
    sleep ; stall RAW                                   ; pc=0x0FE4
    sleep ; stall RAW                                   ; pc=0x0FE8
    sleep ; stall RAW                                   ; pc=0x0FEC
    add x6, x5, x3                                      ; pc=0x0FF0
    sleep ; stall RAW                                   ; pc=0x0FF4
    sleep ; stall RAW                                   ; pc=0x0FF8
    sleep ; stall RAW                                   ; pc=0x0FFC
    sw x6, -32(x17) ; mid1                              ; pc=0x1000
    lw x6, -12(x17) ; v0                                ; pc=0x1004
    sleep ; stall RAW                                   ; pc=0x1008
    sleep ; stall RAW                                   ; pc=0x100C
    sleep ; stall RAW                                   ; pc=0x1010
    addi x3, x0, 3                                      ; pc=0x1014
    sleep ; stall RAW                                   ; pc=0x1018
    sleep ; stall RAW                                   ; pc=0x101C
    sleep ; stall RAW                                   ; pc=0x1020
    add x5, x3, x3                                      ; pc=0x1024
    sleep ; stall RAW                                   ; pc=0x1028
    sleep ; stall RAW                                   ; pc=0x102C
    sleep ; stall RAW                                   ; pc=0x1030
    add x5, x5, x5                                      ; pc=0x1034
    sleep ; stall RAW                                   ; pc=0x1038
    sleep ; stall RAW                                   ; pc=0x103C
    sleep ; stall RAW                                   ; pc=0x1040
    lw x4, -8(x17) ; base ref tea_key                   ; pc=0x1044
    sleep ; stall RAW                                   ; pc=0x1048
    sleep ; stall RAW                                   ; pc=0x104C
    sleep ; stall RAW                                   ; pc=0x1050
    add x4, x4, x5                                      ; pc=0x1054
    sleep ; stall RAW                                   ; pc=0x1058
    sleep ; stall RAW                                   ; pc=0x105C
    sleep ; stall RAW                                   ; pc=0x1060
    lw x5, 0(x4)                                        ; pc=0x1064
    sleep ; stall RAW                                   ; pc=0x1068
    sleep ; stall RAW                                   ; pc=0x106C
    sleep ; stall RAW                                   ; pc=0x1070
    addi x3, x0, 5                                      ; pc=0x1074
    sleep ; stall RAW                                   ; pc=0x1078
    sleep ; stall RAW                                   ; pc=0x107C
    sleep ; stall RAW                                   ; pc=0x1080
    srl x4, x6, x3                                      ; pc=0x1084
    sleep ; stall RAW                                   ; pc=0x1088
    sleep ; stall RAW                                   ; pc=0x108C
    sleep ; stall RAW                                   ; pc=0x1090
    add x4, x4, x5                                      ; pc=0x1094
    sleep ; stall RAW                                   ; pc=0x1098
    sleep ; stall RAW                                   ; pc=0x109C
    sleep ; stall RAW                                   ; pc=0x10A0
    sw x4, -36(x17) ; right1                            ; pc=0x10A4
    lw x4, -16(x17) ; v1                                ; pc=0x10A8
    sleep ; stall RAW                                   ; pc=0x10AC
    sleep ; stall RAW                                   ; pc=0x10B0
    sleep ; stall RAW                                   ; pc=0x10B4
    lw x5, -28(x17) ; left1                             ; pc=0x10B8
    sleep ; stall RAW                                   ; pc=0x10BC
    sleep ; stall RAW                                   ; pc=0x10C0
    sleep ; stall RAW                                   ; pc=0x10C4
    lw x6, -32(x17) ; mid1                              ; pc=0x10C8
    sleep ; stall RAW                                   ; pc=0x10CC
    sleep ; stall RAW                                   ; pc=0x10D0
    sleep ; stall RAW                                   ; pc=0x10D4
    xor x3, x5, x6                                      ; pc=0x10D8
    sleep ; stall RAW                                   ; pc=0x10DC
    sleep ; stall RAW                                   ; pc=0x10E0
    sleep ; stall RAW                                   ; pc=0x10E4
    lw x6, -36(x17) ; right1                            ; pc=0x10E8
    sleep ; stall RAW                                   ; pc=0x10EC
    sleep ; stall RAW                                   ; pc=0x10F0
    sleep ; stall RAW                                   ; pc=0x10F4
    xor x5, x3, x6                                      ; pc=0x10F8
    sleep ; stall RAW                                   ; pc=0x10FC
    sleep ; stall RAW                                   ; pc=0x1100
    sleep ; stall RAW                                   ; pc=0x1104
    sub x6, x4, x5                                      ; pc=0x1108
    sleep ; stall RAW                                   ; pc=0x110C
    sleep ; stall RAW                                   ; pc=0x1110
    sleep ; stall RAW                                   ; pc=0x1114
    sw x6, -16(x17) ; v1                                ; pc=0x1118
    lw x6, -16(x17) ; v1                                ; pc=0x111C
    sleep ; stall RAW                                   ; pc=0x1120
    sleep ; stall RAW                                   ; pc=0x1124
    sleep ; stall RAW                                   ; pc=0x1128
    addi x5, x0, 0                                      ; pc=0x112C
    sleep ; stall RAW                                   ; pc=0x1130
    sleep ; stall RAW                                   ; pc=0x1134
    sleep ; stall RAW                                   ; pc=0x1138
    add x4, x5, x5                                      ; pc=0x113C
    sleep ; stall RAW                                   ; pc=0x1140
    sleep ; stall RAW                                   ; pc=0x1144
    sleep ; stall RAW                                   ; pc=0x1148
    add x4, x4, x4                                      ; pc=0x114C
    sleep ; stall RAW                                   ; pc=0x1150
    sleep ; stall RAW                                   ; pc=0x1154
    sleep ; stall RAW                                   ; pc=0x1158
    lw x3, -8(x17) ; base ref tea_key                   ; pc=0x115C
    sleep ; stall RAW                                   ; pc=0x1160
    sleep ; stall RAW                                   ; pc=0x1164
    sleep ; stall RAW                                   ; pc=0x1168
    add x3, x3, x4                                      ; pc=0x116C
    sleep ; stall RAW                                   ; pc=0x1170
    sleep ; stall RAW                                   ; pc=0x1174
    sleep ; stall RAW                                   ; pc=0x1178
    lw x4, 0(x3)                                        ; pc=0x117C
    sleep ; stall RAW                                   ; pc=0x1180
    sleep ; stall RAW                                   ; pc=0x1184
    sleep ; stall RAW                                   ; pc=0x1188
    addi x5, x0, 4                                      ; pc=0x118C
    sleep ; stall RAW                                   ; pc=0x1190
    sleep ; stall RAW                                   ; pc=0x1194
    sleep ; stall RAW                                   ; pc=0x1198
    sll x3, x6, x5                                      ; pc=0x119C
    sleep ; stall RAW                                   ; pc=0x11A0
    sleep ; stall RAW                                   ; pc=0x11A4
    sleep ; stall RAW                                   ; pc=0x11A8
    add x3, x3, x4                                      ; pc=0x11AC
    sleep ; stall RAW                                   ; pc=0x11B0
    sleep ; stall RAW                                   ; pc=0x11B4
    sleep ; stall RAW                                   ; pc=0x11B8
    sw x3, -40(x17) ; left0                             ; pc=0x11BC
    lw x3, -16(x17) ; v1                                ; pc=0x11C0
    sleep ; stall RAW                                   ; pc=0x11C4
    sleep ; stall RAW                                   ; pc=0x11C8
    sleep ; stall RAW                                   ; pc=0x11CC
    lw x4, -20(x17) ; sum                               ; pc=0x11D0
    sleep ; stall RAW                                   ; pc=0x11D4
    sleep ; stall RAW                                   ; pc=0x11D8
    sleep ; stall RAW                                   ; pc=0x11DC
    add x6, x3, x4                                      ; pc=0x11E0
    sleep ; stall RAW                                   ; pc=0x11E4
    sleep ; stall RAW                                   ; pc=0x11E8
    sleep ; stall RAW                                   ; pc=0x11EC
    sw x6, -44(x17) ; mid0                              ; pc=0x11F0
    lw x6, -16(x17) ; v1                                ; pc=0x11F4
    sleep ; stall RAW                                   ; pc=0x11F8
    sleep ; stall RAW                                   ; pc=0x11FC
    sleep ; stall RAW                                   ; pc=0x1200
    addi x4, x0, 1                                      ; pc=0x1204
    sleep ; stall RAW                                   ; pc=0x1208
    sleep ; stall RAW                                   ; pc=0x120C
    sleep ; stall RAW                                   ; pc=0x1210
    add x3, x4, x4                                      ; pc=0x1214
    sleep ; stall RAW                                   ; pc=0x1218
    sleep ; stall RAW                                   ; pc=0x121C
    sleep ; stall RAW                                   ; pc=0x1220
    add x3, x3, x3                                      ; pc=0x1224
    sleep ; stall RAW                                   ; pc=0x1228
    sleep ; stall RAW                                   ; pc=0x122C
    sleep ; stall RAW                                   ; pc=0x1230
    lw x5, -8(x17) ; base ref tea_key                   ; pc=0x1234
    sleep ; stall RAW                                   ; pc=0x1238
    sleep ; stall RAW                                   ; pc=0x123C
    sleep ; stall RAW                                   ; pc=0x1240
    add x5, x5, x3                                      ; pc=0x1244
    sleep ; stall RAW                                   ; pc=0x1248
    sleep ; stall RAW                                   ; pc=0x124C
    sleep ; stall RAW                                   ; pc=0x1250
    lw x3, 0(x5)                                        ; pc=0x1254
    sleep ; stall RAW                                   ; pc=0x1258
    sleep ; stall RAW                                   ; pc=0x125C
    sleep ; stall RAW                                   ; pc=0x1260
    addi x4, x0, 5                                      ; pc=0x1264
    sleep ; stall RAW                                   ; pc=0x1268
    sleep ; stall RAW                                   ; pc=0x126C
    sleep ; stall RAW                                   ; pc=0x1270
    srl x5, x6, x4                                      ; pc=0x1274
    sleep ; stall RAW                                   ; pc=0x1278
    sleep ; stall RAW                                   ; pc=0x127C
    sleep ; stall RAW                                   ; pc=0x1280
    add x5, x5, x3                                      ; pc=0x1284
    sleep ; stall RAW                                   ; pc=0x1288
    sleep ; stall RAW                                   ; pc=0x128C
    sleep ; stall RAW                                   ; pc=0x1290
    sw x5, -48(x17) ; right0                            ; pc=0x1294
    lw x5, -12(x17) ; v0                                ; pc=0x1298
    sleep ; stall RAW                                   ; pc=0x129C
    sleep ; stall RAW                                   ; pc=0x12A0
    sleep ; stall RAW                                   ; pc=0x12A4
    lw x3, -40(x17) ; left0                             ; pc=0x12A8
    sleep ; stall RAW                                   ; pc=0x12AC
    sleep ; stall RAW                                   ; pc=0x12B0
    sleep ; stall RAW                                   ; pc=0x12B4
    lw x6, -44(x17) ; mid0                              ; pc=0x12B8
    sleep ; stall RAW                                   ; pc=0x12BC
    sleep ; stall RAW                                   ; pc=0x12C0
    sleep ; stall RAW                                   ; pc=0x12C4
    xor x4, x3, x6                                      ; pc=0x12C8
    sleep ; stall RAW                                   ; pc=0x12CC
    sleep ; stall RAW                                   ; pc=0x12D0
    sleep ; stall RAW                                   ; pc=0x12D4
    lw x6, -48(x17) ; right0                            ; pc=0x12D8
    sleep ; stall RAW                                   ; pc=0x12DC
    sleep ; stall RAW                                   ; pc=0x12E0
    sleep ; stall RAW                                   ; pc=0x12E4
    xor x3, x4, x6                                      ; pc=0x12E8
    sleep ; stall RAW                                   ; pc=0x12EC
    sleep ; stall RAW                                   ; pc=0x12F0
    sleep ; stall RAW                                   ; pc=0x12F4
    sub x6, x5, x3                                      ; pc=0x12F8
    sleep ; stall RAW                                   ; pc=0x12FC
    sleep ; stall RAW                                   ; pc=0x1300
    sleep ; stall RAW                                   ; pc=0x1304
    sw x6, -12(x17) ; v0                                ; pc=0x1308
    lw x6, -20(x17) ; sum                               ; pc=0x130C
    sleep ; stall RAW                                   ; pc=0x1310
    sleep ; stall RAW                                   ; pc=0x1314
    sleep ; stall RAW                                   ; pc=0x1318
    addiHIGH x5, x0, 0                                  ; pc=0x131C
    sleep ; stall RAW                                   ; pc=0x1320
    sleep ; stall RAW                                   ; pc=0x1324
    sleep ; stall RAW                                   ; pc=0x1328
    addi x5, x5, 32808                                  ; pc=0x132C
    sleep ; stall RAW                                   ; pc=0x1330
    sleep ; stall RAW                                   ; pc=0x1334
    sleep ; stall RAW                                   ; pc=0x1338
    lw x3, 0(x5) ; DELTA                                ; pc=0x133C
    sleep ; stall RAW                                   ; pc=0x1340
    sleep ; stall RAW                                   ; pc=0x1344
    sleep ; stall RAW                                   ; pc=0x1348
    sub x5, x6, x3                                      ; pc=0x134C
    sleep ; stall RAW                                   ; pc=0x1350
    sleep ; stall RAW                                   ; pc=0x1354
    sleep ; stall RAW                                   ; pc=0x1358
    sw x5, -20(x17) ; sum                               ; pc=0x135C
    lw x5, -24(x17) ; i                                 ; pc=0x1360
    sleep ; stall RAW                                   ; pc=0x1364
    sleep ; stall RAW                                   ; pc=0x1368
    sleep ; stall RAW                                   ; pc=0x136C
    addi x3, x0, 1                                      ; pc=0x1370
    sleep ; stall RAW                                   ; pc=0x1374
    sleep ; stall RAW                                   ; pc=0x1378
    sleep ; stall RAW                                   ; pc=0x137C
    add x6, x5, x3                                      ; pc=0x1380
    sleep ; stall RAW                                   ; pc=0x1384
    sleep ; stall RAW                                   ; pc=0x1388
    sleep ; stall RAW                                   ; pc=0x138C
    sw x6, -24(x17) ; i                                 ; pc=0x1390
    jal x0, -1172                                       ; pc=0x1394 ; target=.L2_for_start ; addr=0x0F00
    sleep ; nop despues de control                      ; pc=0x1398
    sleep ; nop despues de control                      ; pc=0x139C
.L3_for_end:

    lw x6, -12(x17) ; v0                                ; pc=0x13A0
    sleep ; stall RAW                                   ; pc=0x13A4
    sleep ; stall RAW                                   ; pc=0x13A8
    sleep ; stall RAW                                   ; pc=0x13AC
    addi x3, x0, 0                                      ; pc=0x13B0
    sleep ; stall RAW                                   ; pc=0x13B4
    sleep ; stall RAW                                   ; pc=0x13B8
    sleep ; stall RAW                                   ; pc=0x13BC
    add x5, x3, x3                                      ; pc=0x13C0
    sleep ; stall RAW                                   ; pc=0x13C4
    sleep ; stall RAW                                   ; pc=0x13C8
    sleep ; stall RAW                                   ; pc=0x13CC
    add x5, x5, x5                                      ; pc=0x13D0
    sleep ; stall RAW                                   ; pc=0x13D4
    sleep ; stall RAW                                   ; pc=0x13D8
    sleep ; stall RAW                                   ; pc=0x13DC
    lw x4, -4(x17) ; base ref v                         ; pc=0x13E0
    sleep ; stall RAW                                   ; pc=0x13E4
    sleep ; stall RAW                                   ; pc=0x13E8
    sleep ; stall RAW                                   ; pc=0x13EC
    add x4, x4, x5                                      ; pc=0x13F0
    sleep ; stall RAW                                   ; pc=0x13F4
    sleep ; stall RAW                                   ; pc=0x13F8
    sleep ; stall RAW                                   ; pc=0x13FC
    sw x6, 0(x4)                                        ; pc=0x1400
    lw x6, -16(x17) ; v1                                ; pc=0x1404
    sleep ; stall RAW                                   ; pc=0x1408
    sleep ; stall RAW                                   ; pc=0x140C
    sleep ; stall RAW                                   ; pc=0x1410
    addi x4, x0, 1                                      ; pc=0x1414
    sleep ; stall RAW                                   ; pc=0x1418
    sleep ; stall RAW                                   ; pc=0x141C
    sleep ; stall RAW                                   ; pc=0x1420
    add x5, x4, x4                                      ; pc=0x1424
    sleep ; stall RAW                                   ; pc=0x1428
    sleep ; stall RAW                                   ; pc=0x142C
    sleep ; stall RAW                                   ; pc=0x1430
    add x5, x5, x5                                      ; pc=0x1434
    sleep ; stall RAW                                   ; pc=0x1438
    sleep ; stall RAW                                   ; pc=0x143C
    sleep ; stall RAW                                   ; pc=0x1440
    lw x3, -4(x17) ; base ref v                         ; pc=0x1444
    sleep ; stall RAW                                   ; pc=0x1448
    sleep ; stall RAW                                   ; pc=0x144C
    sleep ; stall RAW                                   ; pc=0x1450
    add x3, x3, x5                                      ; pc=0x1454
    sleep ; stall RAW                                   ; pc=0x1458
    sleep ; stall RAW                                   ; pc=0x145C
    sleep ; stall RAW                                   ; pc=0x1460
    sw x6, 0(x3)                                        ; pc=0x1464
.L_codegen_3_tea_decrypt_end:
    ; epilogue
    lw x1, 0(x2)                                        ; pc=0x1468
    sleep ; stall RAW                                   ; pc=0x146C
    sleep ; stall RAW                                   ; pc=0x1470
    sleep ; stall RAW                                   ; pc=0x1474
    lw x17, 4(x2)                                       ; pc=0x1478
    sleep ; stall RAW                                   ; pc=0x147C
    sleep ; stall RAW                                   ; pc=0x1480
    sleep ; stall RAW                                   ; pc=0x1484
    addi x2, x2, 60                                     ; pc=0x1488
    sleep ; stall RAW                                   ; pc=0x148C
    sleep ; stall RAW                                   ; pc=0x1490
    sleep ; stall RAW                                   ; pc=0x1494
    jalr x1, 0                                          ; pc=0x1498
    sleep ; nop despues de control                      ; pc=0x149C
    sleep ; nop despues de control                      ; pc=0x14A0