...
; Tabla de etiquetas
;   maximo_lista = 0x0000
;   L_for_start_0 = 0x005C
;   .L_ir_1_ir_cmp_true = 0x007C
;   .L_ir_2_ir_cmp_end = 0x0080
;   .L_ir_3_ir_cmp_true = 0x00BC
;   .L_ir_4_ir_cmp_end = 0x00C0
;   L_else_2 = 0x00E8
;   L_end_if_3 = 0x00E8
;   L_for_end_1 = 0x00F8
;   .L_ir_0_maximo_lista_end = 0x0100

; Referencias resueltas
;   pc=0x0074 blt -> .L_ir_1_ir_cmp_true (addr=0x007C, offset=8)
;   pc=0x0078 jal -> .L_ir_2_ir_cmp_end (addr=0x0080, offset=8)
;   pc=0x0088 beq -> L_for_end_1 (addr=0x00F8, offset=112)
;   pc=0x00B4 blt -> .L_ir_3_ir_cmp_true (addr=0x00BC, offset=8)
;   pc=0x00B8 jal -> .L_ir_4_ir_cmp_end (addr=0x00C0, offset=8)
;   pc=0x00D8 beq -> L_else_2 (addr=0x00E8, offset=16)
;   pc=0x00E4 jal -> L_end_if_3 (addr=0x00E8, offset=4)
;   pc=0x00F4 jal -> L_for_start_0 (addr=0x005C, offset=-152)
;   pc=0x00FC jal -> .L_ir_0_maximo_lista_end (addr=0x0100, offset=4)
...
L_for_start_0:
    ...
    blt x3, x6, 8                                       ; pc=0x0074 ; target=.L_ir_1_ir_cmp_true ; addr=0x007C
    jal x0, 8                                           ; pc=0x0078 ; target=.L_ir_2_ir_cmp_end ; addr=0x0080
.L_ir_1_ir_cmp_true:
    addi x5, x0, 1                                      ; pc=0x007C
.L_ir_2_ir_cmp_end:
    sw x5, -40(x17) ; t2                                ; pc=0x0080
    lw x7, -40(x17) ; t2                                ; pc=0x0084
    beq x7, x0, 112                                     ; pc=0x0088 ; target=L_for_end_1 ; addr=0x00F8
    ...
    blt x4, x6, 8                                       ; pc=0x00B4 ; target=.L_ir_3_ir_cmp_true ; addr=0x00BC
    jal x0, 8                                           ; pc=0x00B8 ; target=.L_ir_4_ir_cmp_end ; addr=0x00C0
.L_ir_3_ir_cmp_true:
...
.L_ir_4_ir_cmp_end:
    ...
    beq x5, x0, 16                                      ; pc=0x00D8 ; target=L_else_2 ; addr=0x00E8
    lw x7, -16(x17) ; num                               ; pc=0x00DC
    add x4, x7, x0 ; promote maximo                     ; pc=0x00E0
    jal x0, 4                                           ; pc=0x00E4 ; target=L_end_if_3 ; addr=0x00E8
L_else_2:
L_end_if_3:
    addi x6, x0, 1                                      ; pc=0x00E8
    add x8, x3, x6                                      ; pc=0x00EC
    add x3, x8, x0 ; promote i                          ; pc=0x00F0
    jal x0, -152                                        ; pc=0x00F4 ; target=L_for_start_0 ; addr=0x005C
L_for_end_1:
    add x11, x4, x0                                     ; pc=0x00F8
    jal x0, 4                                           ; pc=0x00FC ; target=.L_ir_0_maximo_lista_end ; addr=0x0100
