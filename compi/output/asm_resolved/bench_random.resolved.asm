...
;   .L1_while_end = 0x008C
;   .L2_while_start = 0x009C
;   .L4_if_else = 0x00FC
;   .L5_if_end = 0x00FC
;   .L3_while_end = 0x0110
;   .L_codegen_1_main_end = 0x011C

; Referencias resueltas
;   pc=0x0000 portalv -> .L_codegen_0_enderExit (addr=0x0018, offset=24)
;   pc=0x0058 bge -> .L1_while_end (addr=0x008C, offset=52)
;   pc=0x0088 jal -> .L0_while_start (addr=0x0050, offset=-56)
;   pc=0x00A4 bge -> .L3_while_end (addr=0x0110, offset=108)
;   pc=0x00E4 blt -> .L4_if_else (addr=0x00FC, offset=24)
;   pc=0x00F8 jal -> .L5_if_end (addr=0x00FC, offset=4)
;   pc=0x010C jal -> .L2_while_start (addr=0x009C, offset=-112)
;   pc=0x0118 jal -> .L_codegen_1_main_end (addr=0x011C, offset=4)
...
.L0_while_start:
    ...
    bge x3, x4, 52                                      ; pc=0x0058 ; target=.L1_while_end ; addr=0x008C
    ...
    jal x0, -56                                         ; pc=0x0088 ; target=.L0_while_start ; addr=0x0050
.L1_while_end:
...
.L2_while_start:
    ...
    bge x5, x6, 108                                     ; pc=0x00A4 ; target=.L3_while_end ; addr=0x0110
...
.L5_if_end:
    ...
    jal x0, -112                                        ; pc=0x010C ; target=.L2_while_start ; addr=0x009C
.L3_while_end:

    lw x4, -1932(x17) ; suma                            ; pc=0x0110
    add x11, x4, x0                                     ; pc=0x0114
    jal x0, 4                                           ; pc=0x0118 ; target=.L_codegen_1_main_end ; addr=0x011C
