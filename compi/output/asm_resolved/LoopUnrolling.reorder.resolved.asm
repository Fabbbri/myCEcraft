...
;   .L_ir_5_ir_cmp_true = 0x00FC
;   .L_ir_6_ir_cmp_end = 0x0100
;   L_for_end_3 = 0x0178
;   L_for_end_1 = 0x0188
;   .L_ir_2_procesar_bloques_end = 0x0194
...
;   pc=0x00E0 beq -> L_for_end_1 (addr=0x0188, offset=168)
;   pc=0x00F4 blt -> .L_ir_5_ir_cmp_true (addr=0x00FC, offset=8)
;   pc=0x00F8 jal -> .L_ir_6_ir_cmp_end (addr=0x0100, offset=8)
;   pc=0x0108 beq -> L_for_end_3 (addr=0x0178, offset=112)
;   pc=0x0174 jal -> L_for_start_2 (addr=0x00EC, offset=-136)
;   pc=0x0184 jal -> L_for_start_0 (addr=0x00C4, offset=-192)
;   pc=0x0190 jal -> .L_ir_2_procesar_bloques_end (addr=0x0194, offset=4)
...
L_for_start_0:
    ...
    blt x3, x7, 8                                       ; pc=0x00CC ; target=.L_ir_3_ir_cmp_true ; addr=0x00D4
...
.L_ir_4_ir_cmp_end:
    ...
    beq x9, x0, 168                                     ; pc=0x00E0 ; target=L_for_end_1 ; addr=0x0188
    addi x10, x0, 0                                     ; pc=0x00E4
    add x4, x10, x0 ; promote i                         ; pc=0x00E8
L_for_start_2:
    addi x5, x0, 3                                      ; pc=0x00EC
    addi x6, x0, 0                                      ; pc=0x00F0
    blt x4, x5, 8                                       ; pc=0x00F4 ; target=.L_ir_5_ir_cmp_true ; addr=0x00FC
    jal x0, 8                                           ; pc=0x00F8 ; target=.L_ir_6_ir_cmp_end ; addr=0x0100
.L_ir_5_ir_cmp_true:
    addi x6, x0, 1                                      ; pc=0x00FC
.L_ir_6_ir_cmp_end:
    sw x6, -24(x17) ; t1                                ; pc=0x0100
    lw x8, -24(x17) ; t1                                ; pc=0x0104
    beq x8, x0, 112                                     ; pc=0x0108 ; target=L_for_end_3 ; addr=0x0178
    ...
    jal x0, -136                                        ; pc=0x0174 ; target=L_for_start_2 ; addr=0x00EC
L_for_end_3:
    addi x5, x0, 1                                      ; pc=0x0178
    add x8, x3, x5                                      ; pc=0x017C
    add x3, x8, x0 ; promote b                          ; pc=0x0180
    jal x0, -192                                        ; pc=0x0184 ; target=L_for_start_0 ; addr=0x00C4
L_for_end_1:
    lw x6, -8(x17) ; total                              ; pc=0x0188
    add x11, x6, x0                                     ; pc=0x018C
    jal x0, 4                                           ; pc=0x0190 ; target=.L_ir_2_procesar_bloques_end ; addr=0x0194
