...
;   pc=0x0060 beq -> L_for_end_1 (addr=0x0094, offset=52)
;   pc=0x0090 jal -> L_for_start_0 (addr=0x0048, offset=-72)
...
.L_ir_3_ir_cmp_end:
    ...
    beq x6, x0, 52                                      ; pc=0x0060 ; target=L_for_end_1 ; addr=0x0094
    add x8, x4, x3                                      ; pc=0x0064
    add x4, x8, x0 ; promote par                        ; pc=0x0068
    add x9, x5, x3                                      ; pc=0x006C
    sw x9, -24(x17) ; t2                                ; pc=0x0070
    lw x7, -24(x17) ; t2                                ; pc=0x0074
    ...
    add x3, x8, x0 ; promote i                          ; pc=0x008C
    jal x0, -72                                         ; pc=0x0090 ; target=L_for_start_0 ; addr=0x0048
L_for_end_1:
    add x10, x4, x5                                     ; pc=0x0094
    sw x10, -36(x17) ; t5                               ; pc=0x0098
    lw x7, -36(x17) ; t5                                ; pc=0x009C
    add x11, x7, x0                                     ; pc=0x00A0
    jal x0, 4                                           ; pc=0x00A4 ; target=.L_ir_1_main_end ; addr=0x00A8
.L_ir_1_main_end:
    ; epilogue
    lw x17, 4(x2)                                       ; pc=0x00A8
    addi x2, x2, 44                                     ; pc=0x00AC
    freeze                                              ; pc=0x00B0